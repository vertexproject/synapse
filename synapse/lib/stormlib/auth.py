...
class UserProfile(s_stormtypes.Dict):
    ...
    def __len__(self):
        mesg = f'{self._storm_typename} does not support length calculation.'
    ...
    async def _getValu(self, key, default=s_common.novalu):
        key = await s_stormtypes.tostr(key)
        if self.runt.user.iden != self.valu:
            self.runt.confirm(('auth', 'user', 'profile', 'get', key))
        return await self.runt.view.core.getUserProfInfo(self.valu, key, default=default)
...
@s_stormtypes.registry.registerType
class UserJson(s_stormtypes.Dict):
    '''
    ...
    async def _getData(self):
        self._confirm('get')
        return {path: item async for path, item in self.runt.view.core.getJsonObjs(self._fullpath())}

    async def _getValu(self, key, default=s_common.novalu):
        key = await s_stormtypes.tostr(key)
        fullpath = self._fullpath(tuple(key.split('/')))
        self._confirm('get')
        if not await self.runt.view.core.hasJsonObj(fullpath):
            return default
...
@s_stormtypes.registry.registerType
class UserVars(s_stormtypes.Dict):
    '''
    ...
    async def _getData(self):
        return {name: valu async for name, valu in self.runt.view.core.iterUserVars(self.valu)}

    async def _getValu(self, key, default=s_common.novalu):
        key = await s_stormtypes.tostr(key)
        return await self.runt.view.core.getUserVarValu(self.valu, key, default=default)
...
class User(s_stormtypes.Prim):
    ...
    async def _methUserGet(self, name):
        ...
        udef = await self.runt.view.core.getUserDef(self.valu, packroles=packroles)
...
class LibUsers(s_stormtypes.Lib):
    ...
    async def _methUsersList(self):
        return [User(self.runt, useriden) async for useriden in self.runt.view.core.getUserIdens()]
    ...
    async def _methUsersGet(self, iden=None):
        ...
        if self.runt.view.core.hasUserIden(iden):
            return User(self.runt, iden)

    @s_stormtypes.stormfunc(readonly=True)
    async def _methUsersByName(self, name):
        name = await s_stormtypes.tostr(name)
        useriden = await self.runt.view.core.getUserIdenByName(name)
    ...
    async def _methUsersByEmail(self, email):
        email = await s_stormtypes.tostr(email)
        useriden = await self.runt.view.core.getUserIdenByEmail(email)
        if useriden is not None:
            return User(self.runt, useriden)
...
class LibRoles(s_stormtypes.Lib):
    ...
    async def _methRolesList(self):
        return [Role(self.runt, roleiden) async for roleiden in self.runt.view.core.getRoleIdens()]

    @s_stormtypes.stormfunc(readonly=True)
    async def _methRolesGet(self, iden):
        if self.runt.view.core.hasRoleIden(iden):
            return Role(self.runt, iden)

    @s_stormtypes.stormfunc(readonly=True)
    async def _methRolesByName(self, name):
        name = await s_stormtypes.tostr(name)
        roleiden = await self.runt.view.core.getRoleIdenByName(name)
