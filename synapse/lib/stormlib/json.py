...
class JsonSchema(s_stormtypes.StormType):
    ...
    async def _validate(self, item):
        ...
        try:
            result = await s_coro._boundedforked(runJsSchema, self.schema, item, use_default=self.use_default)
...
class JsonLib(s_stormtypes.Lib):
    ...
    async def _jsonSchema(self, schema, use_default=True):
        ...
        try:
            await s_coro._boundedforked(compileJsSchema, schema, use_default=use_default)
