...
class LibEasyPerm(s_stormtypes.Lib):
    ...
    async def _initEasyPerm(self, edef=None, worldreadable=True):
        edef = await s_stormtypes.toprim(edef)
        if edef is None:
            edef = {}

        worldreadble = await s_stormtypes.tobool(worldreadable)

        if not isinstance(edef, dict):
            raise s_exc.BadArg(mesg='Object to add easy perms to must be a dictionary.')

        self.runt.snap.core._initEasyPerm(edef, worldreadable=worldreadable)
