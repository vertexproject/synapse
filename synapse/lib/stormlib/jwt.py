...
class LibJwt(s_stormtypes.Lib):
         ...
         'type': {'type': 'function', '_funcname': '_verify',
                  'args': (
                      ...
                      {'name': 'ssl_opts', 'type': 'dict', 'default': None,
                       'desc': 'SSL options for the jwks_uri fetch (same shape as $lib.inet.http).'},
    ...
    async def _fetchJwks(self, url, host, addrs, ssl_opts, proxy):
        # HTTPS GET of a JWKS document from the pre-vetted addresses; returns the parsed set.
        sslctx = self.runt.view.core.getCachedSslCtx(opts=ssl_opts)
    ...
    async def _getJwks(self, url, ssl_opts, proxy, allowinternal, refresh):
        ...
        if not refresh:
            entry = cache.get(url)
            if entry is not None and s_common.now() / 1000000 < entry[0]:
                return entry[1]
        ...
        async with lock:
            ...
            try:
                jwkset = await self._fetchJwks(url, host, addrs, ssl_opts, proxy)
            except Exception:
                entry = cache.get(url)
                if entry is not None:
                    return entry[1]
            ...
            cache[url] = (s_common.now() / 1000000 + JWKS_TTL, jwkset)
    ...
    async def _resolveKey(self, key, header, alg, jwks_uri, ssl_opts, proxy, allowinternal):
        ...
        jwkset = await self._getJwks(jwks_uri, ssl_opts, proxy, allowinternal, refresh=False)
        try:
            ...
            jwkset = await self._getJwks(jwks_uri, ssl_opts, proxy, allowinternal, refresh=True)
            return _selectJwk(jwkset, header, alg)
    ...
    async def _verify(self, token, key, algorithms, audience=None, issuer=None, subject=None,
                      ...
                      jwks_uri=None, ssl_opts=None, proxy=True, allowinternal=False):
        ...
        try:
            ...
            keybyts = await self._resolveKey(key, header, alg, jwks_uri, ssl_opts, proxy, allowinternal)
            ...
            # s_common.now() is microseconds; convert to epoch seconds for NumericDate math.
            nowsecs = s_common.now() / 1000000
