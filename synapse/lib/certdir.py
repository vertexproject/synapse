...
class CertDir:
    ...
    def _genCertBuilder(self, name: str, pubkey: c_types.PublicKeyTypes) -> c_x509.CertificateBuilder:
        ...
        if not 1 <= len(name.encode('utf-8')) <= 64:
            mesg = f'Certificate name values must be between 1-64 characters when utf8-encoded. got name={name}, len={len(name.encode("utf-8"))}'
    ...
    def _genPkeyCsr(self, name: str, mode: str, outp: OutPutOrNone = None) -> bytes:
        ...
        if not 1 <= len(name.encode('utf-8')) <= 64:
            mesg = f'CSR name values must be between 1-64 characters when utf8-encoded. got name={name}, len={len(name.encode("utf-8"))}'
            raise s_exc.CryptoErr(mesg=mesg)
