...
class CortexMcp(s_httpapi.ApiKeyOnlyMixin, CellMcp):
    '''
...
'''.strip()
    ...
    _view_merge_desc = '''
Merge a forked view's changes down into its parent view; defaults to the user's default view.
The view must be a fork whose parent does not require quorum voting. The merge runs in the
background: this returns immediately with the scheduled merge info, and the forked view (and
its top layer) are removed once the merge completes. Do not assume the fork still exists or
that the parent already reflects the merged data the instant this returns; re-query to confirm.
'''.strip()
    ...
    async def _startStormCursor(self, query, opts):
        ...
        # is a remote cortex proxy); resolve the underlying async iterator once so we can both
        # iterate and explicitly close it. A GenrIter starts the remote task on __aiter__, so it
        # must be entered exactly once.
        genr = aiter(core.storm(query, opts=opts))
    ...
    async def view_list(self):
        ...
        for $view in $lib.view.list() {
            if $lib.auth.users.get().allowed("view.read", gateiden=$view.iden) {
