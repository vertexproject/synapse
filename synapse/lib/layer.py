...
INDX_NORM = b'\x00\x00'
INDX_ARRAY = b'\x00\x01'

INDX_IVAL_MIN = b'\x00\x02'
INDX_IVAL_MAX = b'\x00\x03'
INDX_IVAL_DURATION = b'\x00\x04'

INDX_TAGFORM_MIN = b'\x00\x05'
INDX_TAGFORM_MAX = b'\x00\x06'
INDX_TAGFORM_DURATION = b'\x00\x07'
...
class IndxByPropIvalMax(IndxBy):
    ...
    def __init__(self, layr, form, prop):
        ...
        '''
        abrv = layr.getPropAbrv(form, prop)
        IndxBy.__init__(self, layr, INDX_IVAL_MAX + abrv, db=layr.bypropspec)
...
class IndxByPropIvalDuration(IndxBy):
    ...
    def __init__(self, layr, form, prop):
        ...
        '''
        abrv = layr.getPropAbrv(form, prop)
        IndxBy.__init__(self, layr, INDX_IVAL_DURATION + abrv, db=layr.bypropspec)
...
class IndxByTagIvalMin(IndxBy):
    ...
    def __init__(self, layr, form, tag):
        ...
        if form is not None:
            abrv += layr.getPropAbrv(form, None)
            IndxBy.__init__(self, layr, INDX_TAGFORM_MIN + abrv, db=layr.bytagspec)
        else:
            IndxBy.__init__(self, layr, INDX_IVAL_MIN + abrv, db=layr.bytagspec)
...
class IndxByTagIvalMax(IndxBy):
    ...
    def __init__(self, layr, form, tag):
        ...
        if form is not None:
            abrv += layr.getPropAbrv(form, None)
            IndxBy.__init__(self, layr, INDX_TAGFORM_MAX + abrv, db=layr.bytagspec)
        else:
            IndxBy.__init__(self, layr, INDX_IVAL_MAX + abrv, db=layr.bytagspec)
...
class IndxByTagIvalDuration(IndxBy):
    ...
    def __init__(self, layr, form, tag):
        ...
        if form is not None:
            abrv += layr.getPropAbrv(form, None)
            IndxBy.__init__(self, layr, INDX_TAGFORM_DURATION + abrv, db=layr.bytagspec)
        else:
            IndxBy.__init__(self, layr, INDX_IVAL_DURATION + abrv, db=layr.bytagspec)
...
class IndxByTagPropIvalMax(IndxBy):
    ...
    def __init__(self, layr, form, prop):
        ...
        '''
        abrv = layr.getTagPropAbrv(form, tag, prop)
        IndxBy.__init__(self, layr, INDX_IVAL_MAX + abrv, db=layr.bytagpropspec)
...
class IndxByTagPropIvalDuration(IndxBy):
    ...
    def __init__(self, layr, form, prop):
        ...
        '''
        abrv = layr.getTagPropAbrv(form, tag, prop)
        IndxBy.__init__(self, layr, INDX_IVAL_DURATION + abrv, db=layr.bytagpropspec)
...
class StorTypeIval(StorType):
    ...
    async def _liftIvalAt(self, liftby, valu, reverse=False):
        ...
        for lkey, buid in scan(pkeymin, pkeymax):
...
class Layer(s_nexus.Pusher):
    ...
    async def liftByTagValu(self, tag, cmpr, valu, form=None, reverse=False):
        ...
        async for indx, buid in self.stortypes[STOR_TYPE_TAG_IVAL].indxByTag(tag, cmpr, valu, form=form, reverse=reverse):

            sode = self._getStorNode(buid)
            if sode is None: # pragma: no cover
                continue

            if filt is None or filt(sode['tags'].get(tag)):
    ...
    async def liftByTagPropValu(self, form, tag, prop, cmprvals, reverse=False):
        ...
        for cmpr, valu, kind in cmprvals:
            ...
            async for indx, buid in self.stortypes[kind].indxByTagProp(form, tag, prop, cmpr, valu, reverse=reverse):
    ...
    async def liftByFormValu(self, form, cmprvals, reverse=False):
        for cmpr, valu, kind in cmprvals:
            ...
            async for indx, buid in self.stortypes[kind].indxByForm(form, cmpr, valu, reverse=reverse):
                sode = self._getStorNode(buid)
                if sode is None: # pragma: no cover
    ...
    async def liftByPropValu(self, form, prop, cmprvals, reverse=False):
        for cmpr, valu, kind in cmprvals:
            ...
            async for indx, buid in self.stortypes[kind].indxByProp(form, prop, cmpr, valu, reverse=reverse):
    ...
    async def liftByPropArray(self, form, prop, cmprvals, reverse=False):
        for cmpr, valu, kind in cmprvals:
            async for indx, buid in self.stortypes[kind].indxByPropArray(form, prop, cmpr, valu, reverse=reverse):
                sode = self._getStorNode(buid)
                if sode is None: # pragma: no cover
    ...
    def _editPropSet(self, buid, form, edit, sode, meta):
        ...
        if oldv is not None:
            ...
            if oldt & STOR_FLAG_ARRAY:
                ...
                if oldt == STOR_TYPE_IVAL:
                    ...
                    self.layrslab.delete(INDX_IVAL_DURATION + abrv + dura, buid, db=self.bypropspec)
                    if univabrv is not None:
                        self.layrslab.delete(INDX_IVAL_DURATION + abrv + dura, buid, db=self.bypropspec)

                    if not oldv[1] == valu[1]:
                        oldi = oldi[8:]
                        self.layrslab.delete(INDX_IVAL_MAX + abrv + oldi, buid, db=self.bypropspec)
                        if univabrv is not None:
                            self.layrslab.delete(INDX_IVAL_MAX + univabrv + oldi, buid, db=self.bypropspec)
        ...
        if stortype & STOR_FLAG_ARRAY:
            ...
            if stortype == STOR_TYPE_IVAL:
                ...
                kvpairs.append((INDX_IVAL_DURATION + abrv + dura, buid))
                if univabrv is not None:
                    kvpairs.append((INDX_IVAL_DURATION + univabrv + dura, buid))

                if oldv is None or oldv[1] != valu[1]:
                    indx = indx[8:]
                    kvpairs.append((INDX_IVAL_MAX + abrv + indx, buid))
                    if univabrv is not None:
                        kvpairs.append((INDX_IVAL_MAX + univabrv + indx, buid))

                self.layrslab.putmulti(kvpairs, db=self.bypropspec)
    ...
    def _editPropDel(self, buid, form, edit, sode, meta):
        ...
        if stortype & STOR_FLAG_ARRAY:
            ...
            if stortype == STOR_TYPE_IVAL:
                ...
                self.layrslab.delete(INDX_IVAL_MAX + abrv + indx, buid, db=self.bypropspec)
                self.layrslab.delete(INDX_IVAL_DURATION + abrv + dura, buid, db=self.bypropspec)
                if univabrv is not None:
                    self.layrslab.delete(INDX_IVAL_MAX + univabrv + indx, buid, db=self.bypropspec)
                    self.layrslab.delete(INDX_IVAL_DURATION + univabrv + dura, buid, db=self.bypropspec)
    ...
    def _editTagSet(self, buid, form, edit, sode, meta):
        ...
        tagformabrv = tagabrv + self.setPropAbrv(form, None)
        ...
        if oldv is not None:
            ...
            if oldv != (None, None):
                ...
                self.layrslab.delete(INDX_IVAL_DURATION + tagabrv + dura, buid, db=self.bytagspec)
                self.layrslab.delete(INDX_TAGFORM_DURATION + tagformabrv + dura, buid, db=self.bytagspec)

                if not oldv[0] == valu[0]:
                    indx = self.ivaltimetype.getIntIndx(oldv[0])
                    self.layrslab.delete(INDX_IVAL_MIN + tagabrv + indx, buid, db=self.bytagspec)
                    self.layrslab.delete(INDX_TAGFORM_MIN + tagformabrv + indx, buid, db=self.bytagspec)

                if not oldv[1] == valu[1]:
                    indx = self.ivaltimetype.getIntIndx(oldv[1])
                    self.layrslab.delete(INDX_IVAL_MAX + tagabrv + indx, buid, db=self.bytagspec)
                    self.layrslab.delete(INDX_TAGFORM_MAX + tagformabrv + indx, buid, db=self.bytagspec)

        sode['tags'][tag] = valu
        self.setSodeDirty(buid, sode, form)

        self.layrslab.put(tagformabrv, buid, db=self.bytag)
        ...
        if valu != (None, None):
            ...
            kvpairs.append((INDX_IVAL_DURATION + tagabrv + dura, buid))
            kvpairs.append((INDX_TAGFORM_DURATION + tagformabrv + dura, buid))

            if oldv is None or oldv[0] != valu[0]:
                indx = self.ivaltimetype.getIntIndx(valu[0])
                kvpairs.append((INDX_IVAL_MIN + tagabrv + indx, buid))
                kvpairs.append((INDX_TAGFORM_MIN + tagformabrv + indx, buid))

            if oldv is None or oldv[1] != valu[1]:
                indx = self.ivaltimetype.getIntIndx(valu[1])
                kvpairs.append((INDX_IVAL_MAX + tagabrv + indx, buid))
                kvpairs.append((INDX_TAGFORM_MAX + tagformabrv + indx, buid))

            self.layrslab.putmulti(kvpairs, db=self.bytagspec)
    ...
    def _editTagDel(self, buid, form, edit, sode, meta):
        ...
        self.layrslab.delete(tagformabrv, buid, db=self.bytag)
    ...
    def _editTagPropSet(self, buid, form, edit, sode, meta):
        ...
        if tp_dict:
            ...
            if oldv is not None:
                ...
                if oldt == STOR_TYPE_IVAL:
                    ...
                    self.layrslab.delete(INDX_IVAL_DURATION + tp_abrv + dura, buid, db=self.bytagpropspec)
                    self.layrslab.delete(INDX_IVAL_DURATION + ftp_abrv + dura, buid, db=self.bytagpropspec)
        ...
        if stortype == STOR_TYPE_IVAL:
            ...
            kvpairs.append((INDX_IVAL_DURATION + tp_abrv + dura, buid))
            kvpairs.append((INDX_IVAL_DURATION + ftp_abrv + dura, buid))

            if oldv is None or oldv[1] != valu[1]:
                indx = indx[8:]
                kvpairs.append((INDX_IVAL_MAX + tp_abrv + indx, buid))
                kvpairs.append((INDX_IVAL_MAX + ftp_abrv + indx, buid))

            self.layrslab.putmulti(kvpairs, db=self.bytagpropspec)
    ...
    def _editTagPropDel(self, buid, form, edit, sode, meta):
        ...
        if oldt == STOR_TYPE_IVAL:
            ...
            self.layrslab.delete(INDX_IVAL_MAX + tp_abrv + indx, buid, db=self.bytagpropspec)
            self.layrslab.delete(INDX_IVAL_MAX + ftp_abrv + indx, buid, db=self.bytagpropspec)
            self.layrslab.delete(INDX_IVAL_DURATION + tp_abrv + dura, buid, db=self.bytagpropspec)
            self.layrslab.delete(INDX_IVAL_DURATION + ftp_abrv + dura, buid, db=self.bytagpropspec)
