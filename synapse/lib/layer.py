...
class LayerEditBatch:
    '''
    Accumulate node edits which target specific layers and flush them per-layer.

    Edits are coalesced by (layer, buid) so that each buid appears exactly once in the
    nodeedits list handed to a given layer. Each flush issues one call per layer, which
    is one nexus item, making the flush atomic with respect to that layer.
    ...
    def addEdit(self, layriden, buid, formname, edits):
        '''
        ...
        self.editcount += len(edits)

    def size(self):
        return self.editcount

    def popLayerEdits(self):
        '''
        Drain and return the queued edits as a {layriden: {buid: nodeedit}} mapping.

        This is for callers which need to issue the per-layer calls themselves, such as
        to isolate a failure to a single layer.
        '''
        nodeedits = self.nodeedits
    ...
    async def addNodeEdit(self, layriden, edit):
        '''
        Queue a (buid, form, edits) tuple for the given layer, flushing if chunk is reached.
        '''
        buid, formname, edits = edit
        self.addEdit(layriden, buid, formname, edits)

        if self.chunk is not None and self.editcount >= self.chunk:
            await self.flush()

    async def flush(self):
        '''
        Flush the queued edits, one call per layer.

        Returns:
            dict: A mapping of layer iden to the flat edits which were actually applied.
                  Note that saveNodeEdits() returns an entry for every buid it was given,
                  including buids where every edit was a no-op, so those are stripped here.
        '''
        applied = {}
        ...
        async for layriden, layredits in s_coro.pause(self.nodeedits.items()):
            ...
            if self.nolift:
                await layer.storNodeEditsNoLift(nodeedits, self.meta)
                continue

            _, changes = await layer.saveNodeEdits(nodeedits, self.meta)

            changes = [change for change in changes if change[2]]
            if changes:
                applied[layriden] = changes
