...
class MergeCmd(Cmd):
    ...
    async def _checkNodePerms(self, node, sode, runt):
        ...
        for name in sode.get('antiprops', {}).keys():
            full = node.form.prop(name).full
            runt.confirm(('node', 'prop', 'del', full), gateiden=layr1)
        ...
        async with await runt.snap.view.parent.snap(user=runt.user.iden) as snap:
            ...
            async for node, path in genr:
                ...
                if not onlytags or form == 'syn:tag':
                    ...
                    if (valu := sode.get('antivalu')) is not None:
                        if tagfilter and form == 'syn:tag' and tagfilter(valu[0]):
                            continue

                        if not self.opts.apply:
                            valurepr = node.form.type.repr(valu[0])
                            await runt.printf(f'{nodeiden} DELETE {form} = {valurepr}')
                        else:
                ...
                if not onlytags or form == 'syn:tag':
                    ...
                    async for abrv, valu, tomb in layr.iterNodeData(node.nid):
                        ...
                        if tomb:
                            if not self.opts.apply:
                                await runt.printf(f'{nodeiden} {form} DATA delete {name}')
                            else:
                    ...
                    async for abrv, n2nid, tomb in layr.iterNodeEdgesN1(node.nid):
                        ...
                        if tomb:
                            if not self.opts.apply:
                                ...
                                await protonode.delEdge(verb, dest)
                                subs.append((s_layer.EDIT_EDGE_TOMB_DEL, (verb, dest), ()))
                                if len(subs) >= 1000:
                                    await asyncio.sleep(0)
                        else:
                            if not self.opts.apply:
                                ...
                                await protonode.addEdge(verb, dest)
                                subs.append((s_layer.EDIT_EDGE_DEL, (verb, dest), ()))
                                if len(subs) >= 1000:
                                    await asyncio.sleep(0)
                ...
                async for abrv, n2nid, tomb in layr.iterNodeEdgesN1(node.nid):
                    ...
                    if tomb:
                        if not self.opts.apply:
                            ...
                            self.adds.append((s_layer.EDIT_EDGE_DEL, (verb, dest), ()))
                            self.subs[iden].append((s_layer.EDIT_EDGE_TOMB_DEL, (verb, dest), ()))
                            ecnt += 2
                        ...
                        if not self.opts.apply:
                            ...
                            self.adds.append((s_layer.EDIT_EDGE_ADD, (verb, dest), ()))
                            self.subs[iden].append((s_layer.EDIT_EDGE_DEL, (verb, dest), ()))
                            ecnt += 2
