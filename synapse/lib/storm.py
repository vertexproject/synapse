...
class Runtime(s_base.Base):
    ...
    async def getInput(self):
        ...
        for ndef in self.opts.get('ndefs', ()):
            ...
            elif (rform := self.model.form(ndef[0])) is not None and rform.isrunt:
                async for node in self.view.nodesByPropValu(ndef[0], '=', ndef[1]):
                    yield node, self.initPath(node)
...
class Cmd:
    ...
    def getRuntPode(cls, model):
        ...
        return (('syn:cmd', cls.name), {'props': props})
...
class MergeCmd(Cmd):
    ...
    async def execStormCmd(self, runt, genr):
        ...
        async for node, path in genr:
            ...
            if (nodeiden := node.nid) is not None:
                nodeiden = s_common.ehex(nodeiden)
...
class MoveNodesCmd(Cmd):
    ...
    async def execStormCmd(self, runt, genr):
        ...
        async for node, path in genr:
            ...
            await self._moveMeta(node, sodes, meta, delnode, nodeiden)
            await self._moveProps(node, sodes, meta, delnode, nodeiden)
            await self._moveTags(node, sodes, meta, delnode, nodeiden)
            await self._moveTagProps(node, sodes, meta, delnode, nodeiden)
            await self._moveNodeData(node, meta, delnode, nodeiden)
            await self._moveEdges(node, meta, delnode, nodeiden)
    ...
    async def _moveMeta(self, node, sodes, meta, delnode, nodeiden):
    ...
    async def _moveProps(self, node, sodes, meta, delnode, nodeiden):
    ...
    async def _moveTags(self, node, sodes, meta, delnode, nodeiden):
    ...
    async def _moveTagProps(self, node, sodes, meta, delnode, nodeiden):
    ...
    async def _moveNodeData(self, node, meta, delnode, nodeiden):
    ...
    async def _moveEdges(self, node, meta, delnode, nodeiden):
        ...
        async with await s_spooled.Set.anit(dirn=self.runt.view.core.dirn) as uniqset:
            ...
            if len(self.argv) > 0:
                ...
                async for node, path in genr:
                    ...
                    if (valu := node.nid) is None:
                        valu = s_hashitem.hashitem(node.ndef)
