...
class Prop:
    ...
    def __init__(self, name, modl, info, tobj):
        self._prop_name = name
        self._prop_modl = modl
        self._prop_info = info

    def info(self, name):

        valu = self._prop_info.get(name)
        if valu is None:
            valu = self._prop_type.info(name)

        return valu

    def lift(self, text):
        return self._prop_type.lift(self._prop_name, text)
...
class Model:
    ...
    def __init__(self, load=True):
        ...
        item = s_types.Int('int', self, info, {})

        self.addTypeItem(item)

        info = {'doc': 'The base string type.'}
        item = s_types.Str('str', self, info, {})
    ...
    def addTypeItem(self, item):
        self._modl_types[item.name()] = item

    def getType(self, name):
        return self._modl_types.get(name)

    def getProp(self, name):
        return self._modl_props.get(name)

    #def addPropItem(self, item):

    #def addTypeDef(self, tdef):
        #'''
        #'''
        # (name, info, opts)
