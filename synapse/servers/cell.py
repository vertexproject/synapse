...
async def getCell(outp,
    ...
    outp.printf(f'starting {desc}: {celldir}')

    cell = await ctor.anit(celldir)

    try:

        outp.printf(f'...{desc} API (telepath): {telepath}')
        await cell.dmon.listen(telepath)

        outp.printf(f'...{desc} API (https): {httpport}')
        await cell.addHttpsPort(httpport)

        if name:
            outp.printf(f'...{desc} additional share name: {name}')
...
def parse(argv):
    cctor = os.getenv('SYN_UNIV_CELL', 'synapse.lib.cell.Cell')
