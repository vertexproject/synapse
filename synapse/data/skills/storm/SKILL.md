...
- `view_merge` -- merge a forked view's changes down into its parent. The merge runs in the
  background and the forked view is removed once it completes; the view must be a fork without
  parent quorum voting.
...
+{ -> inet:ip +:asn=1234 }      // subquery filter (keep)
...
:ip -> inet:ip                     // property-based pivot
...
An `ival` property holds a time window -- e.g. the `:seen` property (on forms that implement
the `meta:observable` interface) or a tag's timestamps. Set it with a two-element `(min, max)`
tuple (it normalizes and stores as a `(min, max, duration)` triple):
...
inet:ip for $i in (1, 2, 3) { $lib.print($i) }
...
$lib.inet.http.request($meth, $url,
    ...
    ssl=({"verify": $verify}), proxy=$proxy)     // HTTP request (ssl is a dict)
...
```storm
$node.form                                   // form name string (property)
$node.ndef                                   // (form, value) tuple (property)
$node.nid                                    // node id (property; 3.x integer node id)
$node.value                                  // primary value (property)
