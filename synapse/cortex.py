...
class CoreApi(s_cell.CellApi):
    ...
    async def addTrigger(self, condition, query, info, disabled=False):
        ...
        self.user.confirm(('trigger', 'add'), gateiden=wlyr.iden)
    ...
    async def addNodeTag(self, iden, tag, valu=(None, None)):
        ...
        async with await self.cell.snap(user=self.user) as snap:
            ...
            parts = tag.split('.')
            self.user.confirm(('tag:add', *parts), gateiden=snap.wlyr.iden)
    ...
    async def delNodeTag(self, iden, tag):
        ...
        async with await self.cell.snap(user=self.user) as snap:
            ...
            parts = tag.split('.')
            self.user.confirm(('tag:del', *parts), gateiden=snap.wlyr.iden)
    ...
    async def addNodes(self, nodes):
        ...
        for node in nodes:
            ...
            wlyr = self.cell.view.layers[0]
            self.user.confirm(('node:add', formname), gateiden=wlyr.iden)
            done[formname] = True
...
class Cortex(s_cell.Cell):
    ...
    async def delView(self, iden):
        ...
        await self.addCoreXact(('view:del', {'iden': iden}))
    ...
    async def initLayr(self, layrinfo):
        ...
        gate = await self.auth.addAuthGate(layr.iden, 'layer')
