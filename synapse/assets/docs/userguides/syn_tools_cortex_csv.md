...
usage: synapse.tools.cortex.csv [-h] [--logfile LOGFILE] [--csv-header]
                                [--cli] [--debug] (--cortex CORTEX | --test)
                                [--export] [--view VIEW] [--optsfile OPTSFILE]
...
positional arguments:
  stormfile            A Storm script describing how to create nodes from
                       rows.
  csvfiles             CSV files to load.

options:
  -h, --help           show this help message and exit
  --logfile LOGFILE    Set a log file to get JSON lines from the server
                       events.
  --csv-header         Skip the first line from each CSV file.
  --cli                Drop into a cli session after loading data.
  --debug              Enable verbose debug output.
  --cortex, -c CORTEX  The telepath URL for the cortex ( or alias from
                       ~/.syn/aliases ).
  --test, -t           Perform a local CSV ingest against a temporary cortex.
  --export             Export CSV data to file from storm using
                       $lib.csv.emit(...) events.
  --view VIEW          Optional view to work in.
  --optsfile OPTSFILE  Path to an opts file (.yaml) on disk.
