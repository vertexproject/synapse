<a id="syn-tools-axon-put"></a>

# axon.put

The Synapse `axon.put` command can be used to upload files to a storage Axon (see Axon in the [Synapse Devops Guide](../devopsguide.md#devopsguide)) and optionally create an associated [file:bytes](storm_ref_type_specific.md#type-file) node in a Cortex.
...
options:
  ...
  -a, --axon AXON      URL for a target Axon to store files at.
  -c, --cortex CORTEX  URL for a target Cortex to make file:bytes nodes.
  -r, --recursive      Recursively search paths to upload files.
  -t, --tags TAGS      comma separated list of tags to add to the nodes

```

> [!NOTE]
> This tool was previously run using `synapse.tools.pushfile`, which was removed in Synapse 3.0.0. See [CLI Tool Changes](../300_changes/devops-cli-tools.md#vtx_300_devops-cli-tools).
