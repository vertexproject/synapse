...
- `rstorm` and `synapse.tools.utils.rstorm` (the RST documentation pre-processor) and the Sphinx build are removed. Markdown docs are pre-processed with `synapse.tools.utils.mdstorm` (resolving fenced ` ```mdstorm `/` ```mdinclude `/` ```mdautodoc `/etc directives).
