...
- Updated the `biz:product` form to implement the `biz:manufactured` interface,
  which adds the `:model` property.
- Updated the `inet:http:cookie` form to implement the `meta:usable` interface.
- Removed the unused `:univ` property from the `syn:prop` runt form.
- Added the `it:app:sigma:rule` and `it:app:sigma:matched` forms.
- Added the `:edition` property to the `syn:cmd` form, set for a Storm command
  whose definition declares an edition and left unset otherwise.

### Features and Enhancements

- Added the `*min` and `*max` conditional edit operators, which set a property
  to the minimum or maximum of its current and provided values. Supported on
  properties, virtual properties, and tag timestamps.
- Added a Cortex log message when a Storm package is loaded or unloaded,
  including the package name and version as structured log fields.
- Updated the Storm `help` command to format output using Markdown instead of
  RST.
- Added a `$query.text` property to the `storm:query` type to allow retrieving
  the Storm text of an embedded query.
- Added the ability for a Storm library, type, or command to declare the
  edition which provides it, and reported that `edition` from the Cortex APIs
  and the generated Storm reference.
- Added a new Storm API, `$lib.model.migration.fuse()`, to merge one node into
  another node and delete the source node. The two nodes must be the same form
  or one form must inherit from the other. This requires global admin
  permissions and operates on the entire Cortex.
- Added a default `User-Agent` identifying the service and its version to the
  outbound HTTP requests Synapse services make, including Storm's
  `$lib.inet.http` and `$lib.axon.wget`/`wput` calls, JWKS and OAuth token
  fetches, and backup-restore-from-URL downloads. A `User-Agent` supplied by
  the caller takes precedence, and persists across a Storm request forwarded to
  the Axon.
- Added `$lib.inet.http.useragent` to Storm, which returns the Cortex's default
  outbound HTTP `User-Agent` so a caller may build on it rather than replace
  it.
- Updated the default `User-Agent` the Storm `wget` command sends from
  `Chrome/92` to `Chrome/148`. Pass `--headers` to set your own headers, or
  `--no-headers` to skip the command's defaults -- in which case the request
  carries the Cortex's own default `User-Agent`.
- Added line numbers to `mddocs` doc build validation failures.
- Added `$lib.view.get().getTagCount()` to count the tag rows in a View for a
  given tag and optional form.

### Bugfixes

- Fixed a bug where edits in a forked view could fire extra triggers in some
  cases when adding or removing tombstones.
- Storm expression arithmetic on a typed value now operates on the underlying
  value rather than converting it to a `number`, so `/` on an integer type such
  as `time` is integer division. Values stored as `hugenum` still produce a
  `number`.
- Fixed a bug where `$lib.axon.wput()` confirmed only the `axon.get` permission
  before sending a file from the Axon to a caller specified URL. It now also
  requires `axon.wput`, which `$lib.inet.http` requests specifying a field with
  a SHA256 hash already require for the same operation. The `axon.wput`
  permission is now included in the Cortex permission list.
- Fixed a `TypeError` when setting a tag timestamp virtual property on a tag
  which had no timestamps.
- Fixed a bug where a Cortex write which was in flight during a leadership
  change could hang, and could forward locally calculated edits to the new
  leader. Such a write now raises a `BadState` error.
- Fixed a bug where a Cortex write cancelled while its nexus edit was being
  applied could release the nexus lock twice, raising a `RuntimeError` and
  leaving the lock free while the apply was still in flight.
- Fixed the documentation link in the free space warning message.
- Fixed the leading indentation of a Storm command description leaking into its
  rendered `--help` text.
- Fixed a bug in AHA where a service which had already reconnected under a new
  session could be announced and logged as offline, and its cached connection
  torn down, while it was still online.
- Fixed rendering a doubled blank line between Storm commands and after the
  section intro in generated documentation.
- Fixed a bug in `synapse.tools.cortex.layer.dump` where a layer with a single
  unexported node edit reported that there were no edits to export.
...
- Fixed mdautodoc prepending an extra blank line to Storm command `--help`
  output in rendered documentation.
- Fixed the `$lib.undef` sentinel having no stable repr, so a Storm command
  declaring it as an argument default renders `$lib.undef` in its help text
  rather than an object address which differed on every run.
- Fixed a bug in `synapse.lib.scrape` where the `it:sec:cve` rule dropped every
  other CVE in a comma- or space-delimited list of CVEs.

### Improved documentation

- Removed an outdated section from the Devops Guide about migrating to using
  AHA, as Synapse 3 deployments now require using AHA.

### Deprecations

- Deprecated the `.value` virtual property. It will be removed on or after
  2026-09-24.
