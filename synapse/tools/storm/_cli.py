...
class StormCli(s_cli.Cli, s_printer.StormPrinter):
    ...
    async def runCmdLine(self, line, opts=None):
        # opts allows callers (e.g. rstorm doc generation) to inject Storm options without embedding them in the line.
        # Lines starting with '!' are routed to the base CLI handler; everything else is executed as Storm.
        if line[0] == '!':
