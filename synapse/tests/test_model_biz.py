...
class BizModelTest(s_t_utils.SynTest):
    ...
    async def test_model_biz(self):
        ...
        async with self.getTestCore() as core:
            ...
            await core.nodes('''
                biz:product:name=wootwoot
                    [
                        +(has)> {[ econ:lineitem=* :count=1 ]}
                        +(has)> {[ biz:product=* :name=widgetpart ]}
                    ]
            ''')
            self.len(1, await core.nodes('biz:product:name=wootwoot -(has)> econ:lineitem'))
            self.len(1, await core.nodes('biz:product:name=wootwoot -(has)> biz:product +:name=widgetpart'))
            self.len(2, await core.nodes('biz:product:name=wootwoot -(has)> *'))
