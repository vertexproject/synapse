...
class FakeRpcHandler(s_jsrpc.JsonRpcHandler):
    ...
    async def badinfo(self):
        raise s_exc.BadArg(mesg='badinfo', obj=b'\xff\xfe')
...
class JsRpcTest(s_tests.SynTest):
    ...
    async def _post(self, sess, url, item=None, data=None):
        kwargs = {}
        if item is not None:
            kwargs['json'] = item
    ...
    async def _call(self, sess, url, item):
        status, body = await self._post(sess, url, item=item)
