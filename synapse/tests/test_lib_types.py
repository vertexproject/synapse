...
class TypesTest(s_t_utils.SynTest):
    ...
    async def test_loc(self):
        ...
        async with self.getTestCore() as core:
            ...
            await self.agenlen(11, core.eval('testint +:loc^=u'))
            await self.agenlen(11, core.eval('testint +:loc^=us'))
            await self.agenlen(9, core.eval('testint +:loc^=us.'))
            await self.agenlen(7, core.eval('testint +:loc^=us.v'))
            await self.agenlen(7, core.eval('testint +:loc^=us.va'))
            await self.agenlen(6, core.eval('testint +:loc^=us.va.'))
            await self.agenlen(4, core.eval('testint +:loc^=us.va.fair'))
            ...
            await self.agenlen(12, core.eval('testint +:loc^=""'))
