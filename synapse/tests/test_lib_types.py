...
class TypesTest(s_t_utils.SynTest):
    ...
    async def test_loc(self):
        ...
        async with self.getTestCore() as core:
            ...
            await self.agenlen(1, core.eval('[testint=8 :loc=us.va]'))
            await self.agenlen(1, core.eval('[testint=9 :loc=us]'))
            ...
            await self.agenlen(9, core.eval('testint +:loc^=u'))
            await self.agenlen(9, core.eval('testint +:loc^=us'))
            await self.agenlen(9, core.eval('testint +:loc^=us.'))
            await self.agenlen(5, core.eval('testint +:loc^=us.v'))
            await self.agenlen(5, core.eval('testint +:loc^=us.va'))
            await self.agenlen(5, core.eval('testint +:loc^=us.va.'))
            await self.agenlen(2, core.eval('testint +:loc^=us.va.fair'))
            ...
            await self.agenlen(10, core.eval('testint +:loc^=""'))
