...
class StormlibCompressionTest(s_test.SynTest):
    ...
    async def test_storm_lib_bytes_bzip2(self):
        async with self.getTestCore() as core:
            ...
            await core.nodes(f'[ it:log:event=(node1,) :data="{tenc}" ]')
            await core.nodes(f'[ it:log:event=(node2,) :data="{text}" ]')

            q = 'it:log:event=(node1,) return($lib.compression.bzip2.un($lib.base64.decode(:data)).decode())'
            self.eq(text, await core.callStorm(q))

            q = 'it:log:event=(node2,) return($lib.base64.encode($lib.compression.bzip2.en((:data).encode())))'
    ...
    async def test_storm_lib_bytes_gzip(self):
        async with self.getTestCore() as core:
            ...
            q = 'it:log:event=(node1,) return($lib.compression.gzip.un($lib.base64.decode(:data)).decode())'
            self.eq(text, await core.callStorm(q))

            q = 'it:log:event=(node2,) return($lib.compression.gzip.en((:data).encode()))'
    ...
    async def test_storm_lib_bytes_zlib(self):
        async with self.getTestCore() as core:
            ...
            q = 'it:log:event=(node1,) return($lib.compression.zlib.un($lib.base64.decode(:data)).decode())'
            self.eq(text, await core.callStorm(q))

            q = 'it:log:event=(node2,) return($lib.base64.encode($lib.compression.zlib.en((:data).encode())))'
            self.eq(tenc, await core.callStorm(q))
