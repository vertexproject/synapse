...
class ViewTest(s_t_utils.SynTest):
    ...
    async def test_view_run_node_edits(self):

        # runNodeEdits() fires the consequences of edits which were written straight to a
        # layer rather than through a view's write layer. See NodeFuser for the caller.
        async with self.getTestCore() as core:
            ...
            tdef = {'cond': 'node:del', 'form': 'test:str', 'storm': '$lib.queue.gen(rneq).put(del)'}
            await core.callStorm('return($lib.trigger.add($tdef))', opts={'vars': {'tdef': tdef}})

            tdef = {'cond': 'prop:set', 'prop': 'test:str:hehe', 'storm': '$lib.queue.gen(rneq).put(set)'}
            await core.callStorm('return($lib.trigger.add($tdef))', opts={'vars': {'tdef': tdef}})

            await core.nodes('[ test:str=rne ]')
            buid = (await core.nodes('test:str=rne'))[0].buid

            stortype = core.model.form('test:str').type.stortype

            # an empty edit list is a no-op, and sodecache defaults to empty
            await view.runNodeEdits([], useriden)
            self.eq(0, await core.callStorm('return($lib.queue.gen(rneq).size())'))

            # a prop set which really is visible fires its consequence
            await layr.saveNodeEdits([
                (buid, 'test:str', ((s_layer.EDIT_PROP_SET, ('hehe', 'woot', None, stortype), ()),)),
            ], {'user': useriden})

            await view.runNodeEdits([
                (buid, 'test:str', ((s_layer.EDIT_PROP_SET, ('hehe', 'woot', None, stortype), ()),)),
            ], useriden)

            self.eq(1, await core.callStorm('return($lib.queue.gen(rneq).size())'))

            # a prop set which is not actually visible in the view is filtered out
            await view.runNodeEdits([
                (buid, 'test:str', ((s_layer.EDIT_PROP_SET, ('hehe', 'newp', None, stortype), ()),)),
            ], useriden)

            self.eq(1, await core.callStorm('return($lib.queue.gen(rneq).size())'))

            # a node add is only observable if the buid was not resolvable before
            tdef = {'cond': 'node:add', 'form': 'test:str', 'storm': '$lib.queue.gen(rneq).put(add)'}
            await core.callStorm('return($lib.trigger.add($tdef))', opts={'vars': {'tdef': tdef}})

            addedit = [(buid, 'test:str', ((s_layer.EDIT_NODE_ADD, ('rne', stortype), ()),))]

            await view.runNodeEdits(addedit, useriden)
            self.eq(1, await core.callStorm('return($lib.queue.gen(rneq).size())'))

            await view.runNodeEdits(addedit, useriden, added={buid})
            self.eq(2, await core.callStorm('return($lib.queue.gen(rneq).size())'))

            # a node delete for a buid which is gone and has no cached sodes is skipped,
            # since there is no node to hand to the trigger
            gone = s_common.buid(('test:str', 'nosuchnode'))
            deledit = [(gone, 'test:str', ((s_layer.EDIT_NODE_DEL, ('nosuchnode', stortype), ()),))]

            await view.runNodeEdits(deledit, useriden)
            self.eq(2, await core.callStorm('return($lib.queue.gen(rneq).size())'))

            # with the pre-edit sodes supplied, the node is rebuilt and node:del fires
            sodes = {gone: {layr.iden: {'form': 'test:str', 'valu': ('nosuchnode', stortype)}}}
            await view.runNodeEdits(deledit, useriden, sodecache=sodes)
            self.eq(3, await core.callStorm('return($lib.queue.gen(rneq).size())'))

            # a view whose write layer is read only is skipped, since running the trigger
            # storm would raise IsReadOnly
            ...
            # safe mode skips consequences entirely
            core.safemode = True
            try:
                await view.runNodeEdits(deledit, useriden, sodecache=sodes)
                self.eq(3, await core.callStorm('return($lib.queue.gen(rneq).size())'))
