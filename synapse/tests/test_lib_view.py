...
class ViewTest(s_t_utils.SynTest):
    ...
    async def test_view_getTagsByPrefix(self):
        ...
        async with self.getTestCore() as core:
            ...
            self.len(3, await core.nodes('for $tag in $lib.view.get().getTagsByPrefix(rep) { syn:tag=$tag }'))
            self.len(3, await core.nodes('for $tag in $lib.view.get().getTagsByPrefix(rep) { syn:tag=$tag }', opts=opts))
            self.len(3, await core.nodes('syn:tag^=rep +:depth<=0'))

            self.len(204, await core.nodes('for $tag in $lib.view.get().getTagsByPrefix(rep, depth=1) { syn:tag=$tag }'))
            self.len(204, await core.nodes('for $tag in $lib.view.get().getTagsByPrefix(rep, depth=1) { syn:tag=$tag }', opts=opts))
            self.len(204, await core.nodes('syn:tag^=rep +:depth<=1'))

            self.len(304, await core.nodes('for $tag in $lib.view.get().getTagsByPrefix(rep, depth=2) { syn:tag=$tag }'))
            self.len(304, await core.nodes('for $tag in $lib.view.get().getTagsByPrefix(rep, depth=2) { syn:tag=$tag }', opts=opts))
            self.len(304, await core.nodes('syn:tag^=rep +:depth<=2'))

            nodes0 = await core.nodes('for $tag in $lib.view.get().getTagsByPrefix(b) { syn:tag=$tag }')
            ...
            nodes0 = await core.nodes('for $tag in $lib.view.get().getTagsByPrefix(z) { syn:tag=$tag }')
