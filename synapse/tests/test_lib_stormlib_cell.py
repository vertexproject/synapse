...
class StormCellTest(s_test.SynTest):
    ...
    async def test_stormfix_inet_ssl_cert(self):
        async with self.getTestCore() as core:
            view0 = core.getView().iden
            view1 = await core.callStorm('return($lib.view.get().fork().iden)')
            view2 = await core.callStorm('return($lib.view.add(($lib.layer.add().iden,)).iden)')
            ...
            self.len(1, await core.nodes('''
                $file = {[ file:bytes="guid:00000000000000000000000000000000" +#view0 ]}
                [ inet:ssl:cert=("tcp://1.2.3.4:443", $file) ]
            ''', opts=inview0))

            self.len(1, await core.nodes('''
                $file = {[ file:bytes="guid:00000000000000000000000000000001" +#view1 ]}
                [ inet:ssl:cert=("tcp://1.2.3.4:443", $file) ]
            ''', opts=inview1))

            self.len(1, await core.nodes('''
                $file = {[ file:bytes="guid:00000000000000000000000000000002" +#view2 ]}
                [ inet:ssl:cert=("tcp://1.2.3.4:443", $file) ]
            ''', opts=inview2))
            ...
            msgs = await core.stormlist('$lib.cell.hotFixesCheck()')
            printmesgs = [m[1]['mesg'] for m in msgs if m[0] == 'print']
            self.isin('Would apply fix (4, 0, 0)', printmesgs[0])
            self.eq('', printmesgs[1])
            self.isin('Would apply fix (5, 0, 0)', printmesgs[10])
            self.eq('', printmesgs[11])
            self.isin('This hotfix should', printmesgs[12])
            self.eq('', printmesgs[-1])

            msgs = await core.stormlist('$lib.cell.hotFixesApply()')
            self.stormIsInPrint('Applying hotfix (5, 0, 0) for [Create inet:tls:servercert nodes', msgs)
            self.stormIsInPrint('Applied hotfix (5, 0, 0)', msgs)

            self.len(1, await core.nodes('inet:tls:servercert -> crypto:x509:cert -> file:bytes +#view0', opts=inview0))
            self.len(1, await core.nodes('inet:tls:servercert -> crypto:x509:cert -> file:bytes +#view1', opts=inview1))
            self.len(1, await core.nodes('inet:tls:servercert -> crypto:x509:cert -> file:bytes +#view2', opts=inview2))
