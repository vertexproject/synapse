...
class EconTest(s_utils.SynTest):
    ...
    async def test_model_econ_price_form_virts(self):

        async with self.getTestCore() as core:

            ptype = core.model.type('econ:price')

            # create a test:price node with currency and asof virts
            norm, _ = await ptype.norm('13.37')
            _, norminfo = await ptype.normVirt('currency', norm, 'usd')
            _, norminfo = await ptype.normVirt('asof', norm, '2024', oldvirts=norminfo.get('virts'))
            node = await core.view.addNode('test:price', norm, norminfo=norminfo)
            self.eq(node.ndef, ('test:price', '13.37'))
            self.eq(node.get('.currency'), 'usd')
            self.eq(node.get('.asof'), 1704067200000000)

            # create a second node with different currency
            norm2, _ = await ptype.norm('42.00')
            _, norminfo2 = await ptype.normVirt('currency', norm2, 'eur')
            node2 = await core.view.addNode('test:price', norm2, norminfo=norminfo2)
            self.eq(node2.get('.currency'), 'eur')
            self.none(node2.get('.asof'))

            # lift by form virt
            self.len(1, await core.nodes('test:price.currency=usd'))
            self.len(1, await core.nodes('test:price.currency=eur'))
            self.len(1, await core.nodes('test:price.asof=2024'))

            # filter by form virt (absolute prop name)
            self.len(1, await core.nodes('test:price +test:price.currency=usd'))
            self.len(1, await core.nodes('test:price +test:price.currency=eur'))

            # filter by form virt (relative virt name)
            self.len(1, await core.nodes('test:price +.currency=usd'))
            self.len(1, await core.nodes('test:price +.currency=eur'))
            self.len(0, await core.nodes('test:price +.currency=gbp'))

            # no virts: create a price without setting virts
            nodes = await core.nodes('[ test:price=99.99 ]')
            self.len(1, nodes)
            self.none(nodes[0].get('.currency'))
            self.none(nodes[0].get('.asof'))

            # comparisons with virts
            self.len(1, await core.nodes('test:price=13.37 +test:price>10'))
            self.len(1, await core.nodes('test:price=13.37 +test:price<20'))
