    ...
    async def test_model_econ(self):
        ...
        async with self.getTestCore() as core:
            ...
            nodes = await core.nodes('''
                econ:purchase
                    [
                        +(purchased)> {[ econ:lineitem=* :item={[ biz:product=* :name=widget ]} ]}
                        +(purchased)> {[ biz:product=* :name=gadget ]}
                    ]
            ''')
            self.len(1, nodes)
            self.len(1, await core.nodes('econ:purchase -(purchased)> econ:lineitem'))
            self.len(1, await core.nodes('econ:purchase -(purchased)> biz:product +:name=gadget'))
            self.len(2, await core.nodes('econ:purchase -(purchased)> *'))
