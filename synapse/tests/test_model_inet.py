...
class InetModelTest(s_t_utils.SynTest):
        ...
        async with self.getTestCore() as core:
            ...
            nodes = await core.nodes('''
                [ inet:http:session=*
                    ...
                    :client=tcp://1.2.3.4:1234
                    :server=tcp://5.6.7.8:443
                    :client:host={[ it:host=* :name=httpclient ]}
                    :server:host={[ it:host=* :name=httpserver ]}
                ]
            ''')
            self.propeq(nodes[0], 'cookies', ('baz=faz', 'foo=bar'))
            self.nn(nodes[0].get('period'))
            self.propeq(nodes[0], 'client', 'tcp://1.2.3.4:1234')
        ...
        async with self.getTestCore() as core:
            ...
            nodes = await core.nodes('''
                [ inet:wifi:ap=*
                    ...
                    :encryption:algorithm:name=wpa2
                    :encryption:algorithm={[ meta:algorithm=* :name=wpa2 ]}
                ]
            ''')
            self.len(1, nodes)
            node = nodes[0]
            ...
            self.propeq(node, 'encryption:algorithm:name', 'wpa2')
            self.nn(node.get('encryption:algorithm'))

            self.len(1, await core.nodes('inet:wifi:ap -> geo:place'))
            self.len(1, await core.nodes('inet:wifi:ap -> meta:algorithm'))

    async def test_wifi_session_login(self):
        async with self.getTestCore() as core:

            nodes = await core.nodes('''
                [ inet:wifi:session=*
                    :period=(2024/01/01, 2024/01/02)
                    :server:ap={[ inet:wifi:ap=* :ssid="VertexNet" :bssid=00:11:22:33:44:55 ]}
                    :client:mac=aa:bb:cc:dd:ee:ff
                    :client=tcp://1.2.3.4:1234
                    :server=tcp://5.6.7.8:443
                    :client:host={[ it:host=* :name=laptop ]}
                    :server:host={[ it:host=* :name=router ]}
                ]
            ''')
            self.len(1, nodes)
            node = nodes[0]
            self.eq(node.ndef[0], 'inet:wifi:session')
            self.nn(node.get('period'))
            self.propeq(node, 'client:mac', 'aa:bb:cc:dd:ee:ff')
            self.propeq(node, 'client', 'tcp://1.2.3.4:1234')
            self.propeq(node, 'server', 'tcp://5.6.7.8:443')
            self.nn(node.get('server:ap'))
            self.nn(node.get('client:host'))
            self.nn(node.get('server:host'))

            self.len(1, await core.nodes('inet:wifi:session -> inet:wifi:ap'))
            self.len(1, await core.nodes('inet:wifi:session -> inet:mac'))
            self.len(2, await core.nodes('inet:wifi:session -> it:host'))

            nodes = await core.nodes('''
                [ inet:wifi:login=*
                    :time=2024/01/01
                    :credential={[ auth:passwd=secret ]}
                    :client:host={ it:host:name=laptop }
                    :server:host={[ it:host=* :name=router ]}
            ...
            self.len(1, nodes)
            node = nodes[0]
            self.eq(node.ndef[0], 'inet:wifi:login')
            self.nn(node.get('time'))
            self.nn(node.get('credential'))
            self.nn(node.get('client:host'))
            self.nn(node.get('server:host'))
            self.nn(node.get('session'))
            self.nn(node.get('server:ap'))
            self.propeq(node, 'client:mac', 'aa:bb:cc:dd:ee:ff')

            self.len(1, await core.nodes('inet:wifi:login -> auth:passwd'))
            self.len(2, await core.nodes('inet:wifi:login -> it:host'))
            self.len(1, await core.nodes('inet:wifi:login -> inet:wifi:session'))
            self.len(1, await core.nodes('inet:wifi:login -> inet:wifi:ap'))
            self.len(1, await core.nodes('inet:wifi:login -> inet:mac'))

    async def test_inet_proto_login(self):
        async with self.getTestCore() as core:

            iface = core.model.ifaces.get('inet:proto:login')
            self.nn(iface)

            propnames = {p[0] for p in iface['props']}
            self.eq({'credential', 'session', 'success'}, propnames)

            ifnames = {n for (n, _) in iface.get('interfaces', ())}
            self.isin('inet:proto:request', ifnames)

            iface = core.model.ifaces.get('inet:proto:session')
            self.nn(iface)

            propnames = {p[0] for p in iface['props']}
            self.eq({'client', 'client:host', 'server', 'server:host'}, propnames)

            ifnames = {n for (n, _) in iface.get('interfaces', ())}
            self.isin('base:activity', ifnames)
    ...
    async def test_model_inet_service(self):
        ...
        async with self.getTestCore() as core:
            ...
            self.propeq(nodes[0], 'of', devsgrp.ndef[1], type=devsgrp.ndef[0])
            ...
            [ inet:service:login=*
                ...
                :session={ inet:service:session=$blcksess }
            ...
            for node in nodes:
                ...
                self.propeq(node, 'of', gnrlchan.ndef[1], type=gnrlchan.ndef[0])
