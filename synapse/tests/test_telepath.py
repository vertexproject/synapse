...
class TeleTest(s_t_utils.SynTest):
    ...
    async def test_telepath_opencell(self):
        info = {'path': '/tmp/foo'}
        self.eq(s_telepath._opencell(info), ('/tmp/foo/sock', '*'))

        info = {'path': 'tmp/foo'}
        self.eq(s_telepath._opencell(info), ('tmp/foo/sock', '*'))

        info = {'path': '/foo', 'host': 'tmp'}
        self.eq(s_telepath._opencell(info), ('tmp/foo/sock', '*'))

        info = {'path': '/foo:*/view', 'host': 'tmp'}
        self.eq(s_telepath._opencell(info), ('tmp/foo/sock', '*/view'))

        info = {'path': '/foo:*/view', 'host': 'tmp', 'name': '*/layer'}
        self.eq(s_telepath._opencell(info), ('tmp/foo/sock', '*/layer'))

    async def test_telepath_openunix(self):
        info = {'path': '/tmp/foo/sock'}
        self.eq(s_telepath._openunix(info), ('/tmp/foo/sock', '*'))

        info = {'path': 'tmp/foo/sock'}
        self.eq(s_telepath._openunix(info), ('tmp/foo/sock', '*'))

        info = {'path': '/tmp/foo/sock:*/view'}
        self.eq(s_telepath._openunix(info), ('/tmp/foo/sock', '*/view'))

        info = {'path': '/tmp/foo/sock:*/view', 'name': '*/layer'}
        self.eq(s_telepath._openunix(info), ('/tmp/foo/sock', '*/layer'))

    async def test_telepath_opennetw(self):
        info = {'host': 'localhost'}
        self.eq(s_telepath._opennetw(info), ('localhost', 27492, {}, '*'))

        info = {'host': 'localhost', 'port': 12345}
        self.eq(s_telepath._opennetw(info), ('localhost', 12345, {}, '*'))

        info = {'host': 'localhost', 'path': '/*/view'}
        self.eq(s_telepath._opennetw(info), ('localhost', 27492, {}, '*/view'))

        info = {'host': 'localhost', 'path': '/*/view', 'name': '*/layer'}
        self.eq(s_telepath._opennetw(info), ('localhost', 27492, {}, '*/layer'))

        info = {'host': 'localhost', 'scheme': 'ssl'}
        ret = s_telepath._opennetw(info)
        self.eq(ret[0], 'localhost')
        self.eq(ret[1], 27492)
        self.none(ret[2]['certhash'])
        self.eq(ret[2]['hostname'], 'localhost')
        self.nn(ret[2]['ssl'])
        self.sorteq(ret[2].keys(), ('certhash', 'hostname', 'ssl'))
        self.eq(ret[3], '*')

    async def test_telepath_openinfo(self):
        info = {'scheme': 'aha+consul'}
        with self.raises(s_exc.FeatureNotSupported) as exc:
            await s_telepath.openinfo(info)
        self.eq(exc.exception.get('mesg'), 'Consul is no longer supported.')

        info = {'scheme': 'aha+newp'}
        with self.raises(s_exc.BadUrl) as exc:
            await s_telepath.openinfo(info)
        self.eq(exc.exception.get('mesg'), 'Unknown discovery protocol [newp].')
        self.eq(exc.exception.get('disc'), 'newp')
