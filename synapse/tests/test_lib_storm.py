...
class StormTest(s_t_utils.SynTest):
    ...
    async def test_lib_storm_wildtime(self):

        async with self.getTestCore() as core:

            await core.nodes('[ inet:dns:request=* :time=20211031020202 ]')

            # test * suffix syntax
            self.len(1, await core.nodes('inet:dns:request:time=2021*'))
            self.len(1, await core.nodes('inet:dns:request:time=202110*'))
            self.len(1, await core.nodes('inet:dns:request:time=20211031*'))
            self.len(1, await core.nodes('inet:dns:request:time=2021103102*'))
            self.len(1, await core.nodes('inet:dns:request:time=202110310202*'))
            self.len(1, await core.nodes('inet:dns:request:time=20211031020202*'))

            self.len(0, await core.nodes('inet:dns:request:time=2022*'))
            self.len(0, await core.nodes('inet:dns:request:time=202210*'))
            self.len(0, await core.nodes('inet:dns:request:time=20221031*'))
            self.len(0, await core.nodes('inet:dns:request:time=2022103102*'))
            self.len(0, await core.nodes('inet:dns:request:time=202210310202*'))
            self.len(0, await core.nodes('inet:dns:request:time=20221031020202*'))
            ...
            self.len(0, await core.nodes('inet:dns:request -:time=2021*'))
            self.len(0, await core.nodes('inet:dns:request -:time=202110*'))
            self.len(0, await core.nodes('inet:dns:request -:time=20211031*'))
            self.len(0, await core.nodes('inet:dns:request -:time=2021103102*'))
            self.len(0, await core.nodes('inet:dns:request -:time=202110310202*'))
            self.len(0, await core.nodes('inet:dns:request -:time=20211031020202*'))
            ...
            self.len(1, await core.nodes('inet:dns:request:time=2022-'))
            self.len(1, await core.nodes('inet:dns:request:time=202210-'))
            self.len(1, await core.nodes('inet:dns:request:time=20221031-'))
            self.len(1, await core.nodes('inet:dns:request:time=2022103102-'))
            self.len(1, await core.nodes('inet:dns:request:time=202210310202-'))
            self.len(1, await core.nodes('inet:dns:request:time=20221031020202-'))
            ...
            self.len(0, await core.nodes('inet:dns:request:time=2020*-'))
            self.len(0, await core.nodes('inet:dns:request:time=202010*-'))
            self.len(0, await core.nodes('inet:dns:request:time=20201031*-'))
            self.len(0, await core.nodes('inet:dns:request:time=2020103102*-'))
            self.len(0, await core.nodes('inet:dns:request:time=202010310202*-'))
            self.len(0, await core.nodes('inet:dns:request:time=20201031020202*-'))
