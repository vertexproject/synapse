...
# deterministic iden for the built-in 'user' account created by getTestCluster
testuseriden = s_common.guid('user')
...
testmodel = (
    {
        ...
        'types': (
            ...
            ('test:poly', (('test:int', {}), ('test:str', {})), {}),
            ('test:guidpoly', (('test:guid', {}), ('meta:source', {})), {}),
            ('test:guidsingle', (('test:guid', {}),), {}),
            ('test:int2030', ('int', {'min': 20, 'max': 30}), {}),
            ('test:type10', ('test:type', {}), {
                'props': (

                    ('intprop', ('test:int2030', {}), {}),
                    ('int2', ('int', {}), {}),
                    ('strprop', ('test:lower', {}), {}),
                    ('guidprop', ('guid', {}), {}),
                    ('locprop', ('loc', {}), {}),
            ...
            ('test:lowstr', ('str', {'lower': True}), {'props': ()}),
            ...
            ('test:text', ('text', {}), {'props': ()}),
            ('test:str2', ('test:str', {}), {'props': ()}),
            ('test:baseid', ('str', {}), {'doc': 'A non-form base id type.'}),
            ('test:subid', ('test:baseid', {}), {'props': (), 'doc': 'A form whose type is subof test:baseid.'}),
            ('test:inhstr', ('str', {}), {
                'props': (
                    ('name', ('str', {}), {}),
                ),
            ...
            ('test:strregex', ('str', {'lower': True, 'strip': True, 'regex': r'^#[^\p{Z}#]+$'}), {'props': ()}),
            ('test:migr', ('str', {}), {
                'props': (
                    ('tick', ('test:time', {}), {}),
                ),
            ...
            ('test:guidchild', ('test:guid', {}), {'props': ()}),
            ('test:data', ('data', {}), {
                'props': (
                    ('data', ('test:data', {}), {}),
                ),
            ...
            ('test:zeropad', ('hex', {'zeropad': 20}), {'props': (), 'doc': 'test hex type, zero padded to 40 bytes.'}),
            ...
            ('test:enums:int', ('int', {'enums': ((1, 'fooz'), (2, 'barz'), (3, 'bazz'))}), {'props': ()}),
            ('test:enums:str', ('str', {'enums': 'testx,foox,barx,bazx'}), {'props': ()}),
            ('test:onstorm', ('guid', {}), {
                'on': {'add': {'q': '[ :tick=2025 ]'}},
                'props': (
                    ('tick', ('time', {}), {}),
...
deprmodel = (
    {
        ...
        'types': (
            ...
            ('test:deprform2', ('test:str', {}), {'deprecated': True, 'props': ()}),
            ('test:deprsub', ('str', {}), {
                'props': (
                    ('range', ('test:range', {}), {}),
...
class StubStormSvc(s_stormsvc.StormSvc):
    '''
    A stub storm service for sharing directly on a test daemon rather than
    running a whole cell.

    A cortex identifies a storm service by its cell type and requires it to
    advertise the stormservice feature, so a stub must report both from
    getCellInfo() the way a real cell does. Set ``celltype`` to the name the
    service will be registered under.
    '''
    celltype = None
    ...
    async def getCellInfo(self):
        return {
            ...
            'cell': {'type': self.celltype, 'version': s_version.version},
...
class SynTest(unittest.IsolatedAsyncioTestCase):
    ...
    def checkNode(self, node, expected):
        ...
        model = node.view.core.model
        self.eq(node.ndef[0], ex_ndef[0])
        self.eq(self._striptypes(model, node.form.type, node.ndef[1]), ex_ndef[1])
    ...
    async def checkNodes(self, core, ndefs):
        for ndef in ndefs:
            # normalize the ndef value so callers may pass untyped comp/poly values
            # (norm is idempotent, so already-normalized values are accepted too)
            norm, _ = await core.model.type(ndef[0]).norm(ndef[1])
            node = await core.nodes('', opts={'ndefs': ((ndef[0], norm),)})
            self.len(1, node)
    ...
    async def getTestAha(self, conf=None, dirn=None, ctor=None):
        '''
        Boot a standalone AHA cell for testing. Delegates the actual boot
        ( network/hostname conf defaults, post-boot listen port fixup, and the
        leader self-registration wait ) to synapse.lib.cluster.bootAha(); this
        wrapper only adds test-specific behavior ( nexus replay patching, a
        temp dir, and seeding pre-generated, checked-in certs to skip CA
        generation cost across many test boots ).

        Note:
            Certs pre-made for the default ``synapse`` network are seeded
            unconditionally: AHA's own CA lookup ( AhaCell._initCaCert() ) is
            keyed by the resolved ``aha:network`` name, so seeding them ahead
            of a differently-named network is a no-op -- AHA generates a
            fresh CA for that name instead of using the mismatched one.
        '''
        with self.withNexusReplay():
            ...
            with self.mayTestDir(dirn) as dirn:
                ...
                if not os.path.isdir(dstpath):
                    ...
                    shutil.copytree(srcpath, dstpath, copy_function=self._tinFoilCopy)

                async with s_cluster.bootAha(conf=conf, dirn=dirn, ctor=ctor) as aha:
                    yield aha
    ...
    async def waitCoreStormSvc(self, core, name, timeout=15):
        ...
        '''
        if not await core.waitStormSvc(name, timeout=timeout):
            return None

        return core.getStormSvc(name)

    async def addSvcToCore(self, svc, core, svcname=None):
        '''
        Add a service to a Cortex using telepath over tcp.

        Notes:
            A storm service is registered under its cell type name, which is used
            by default.
        '''
        if svcname is None:
            svcname = svc.getCellType()
    ...
    # maps a service type to the ctor used to boot it in getTestCluster(). values
    # may be dotted-path strings ( resolved lazily to avoid eager imports ) or
    # ctors. Subclasses ( eg the enterprise TstBase ) override for their types.
    testsvc_ctors = {
        'aha': 'synapse.lib.aha.AhaCell',
        'axon': 'synapse.axon.Axon',
        'cortex': 'synapse.cortex.Cortex',
        'jsonstor': 'synapse.lib.jsonstor.JsonStorCell',
    }

    def getTestSvcCtor(self, celltype):
        ctor = self.testsvc_ctors.get(celltype)
        if ctor is None:
            mesg = f'No test service ctor registered for service type: {celltype}'
            raise s_exc.BadArg(mesg=mesg, celltype=celltype)

        if isinstance(ctor, str):
            ctor = s_dyndeps.getDynLocal(ctor)

        return ctor

    @contextlib.asynccontextmanager
    async def getTestCluster(self, svcs=None, dirn=None):
        '''
        ...
        Args:
            svcs (dict): Maps a service type ( eg 'cortex', 'axon', 'search' ) to
                ...
                ``ctor`` ( an explicit cell ctor override ), ``mirrors`` ( the
                number of mirrors to boot alongside the leader; defaults to 0,
                ie leader only ), and ``model`` ( for a 'cortex', whether to load
                the synapse test model; defaults to True ). The special key
                ``aha`` configures the AHA cell via a ``conf`` envelope; an AHA
                network is always created.
            dirn (str): Optional base directory for all services.

        Notes:
            Services locate their peers by cell type via AHA. Requesting a
            'cortex' implicitly also boots an 'axon' and 'jsonstor'. Any deployed
            Storm service is auto-discovered by the Cortex via AHA. Additional
            services may be booted onto the same network afterwards with the
            returned cluster's addSvc() method.

            Every booted Cortex is given known test credentials: the ``root``
            password is set to ``secret`` and a ``user`` account is created with
            email ``user@example.com`` and password ``secret``. Tests should use
            these built-in credentials rather than creating their own.

        Yields:
            Cluster: An object exposing the booted services ( one attribute
            per service type for the leader, plus a ``svcs`` dict keyed by AHA
            name ).

        Notes:
            This is a thin, test-specific wrapper ( known certs, nexus replay
            patching, test cortex credentials/model setup ) over the general
            boot/discover/teardown machinery in synapse.lib.cluster.getCluster().
            Certs pre-made for the default ``synapse`` network are seeded
            unconditionally -- see getTestAha() for why that is safe for any
            other network name too.
        '''
        if svcs is None:
            svcs = {'cortex': {}}
        ...
        hasaha = 'aha' in svcs
        ahaenv = svcs.pop('aha', None)

        # getTestSvcCtor() is the source of truth for a test's service ctors
        # ( eg the enterprise TstBase overrides 'cortex' ) -- getCluster()
        # itself requires an explicit ctor= per entry. Split out the
        # cortex/axon/jsonstor peer group
        # -- booted through getCluster() up front, so the test credentials/
        # model below land before any other requested service ( eg a
        # power-up's onload Storm ) that may depend on the 'user' test
        # account already existing -- from any remaining service types,
        # added afterward via addSvc() once creds are set.
        model = True
        coresvcs = {}
        hascortex = 'cortex' in svcs

        for celltype in ('axon', 'jsonstor', 'cortex'):
            if celltype not in svcs and not (hascortex and celltype in ('axon', 'jsonstor')):
                continue

            env = svcs.pop(celltype, None) or {}
            if celltype == 'cortex':
                model = env.pop('model', True)

            env.setdefault('ctor', self.getTestSvcCtor(celltype))
            coresvcs[celltype] = env
        ...
        with self.withNexusReplay():
            ...
            with self.mayTestDir(dirn) as dirn:
                ...
                dstpath = s_common.genpath(dirn, '000.aha', 'certs')
                if not os.path.isdir(dstpath):
                    srcpath = self.getTestFilePath('aha/certs')
                    shutil.copytree(srcpath, dstpath, copy_function=self._tinFoilCopy)

                async with s_cluster.getCluster(svcs=coresvcs, dirn=dirn) as clus:

                    cell = clus.get('cortex')
                    if cell is not None:
                        # every test Cortex gets known credentials: the root
                        # password is set to 'secret' and a 'user' account
                        # ( user@example.com / 'secret' ) is created.
                        await cell.auth.rootuser.setPasswd('secret')
                        user = await cell.auth.addUser('user', iden=testuseriden,
                                                        email='user@example.com')
                        await user.setPasswd('secret')

                        if model:
                            await cell._addModelDefs(testmodel)

                    for celltype, env in extrasvcs.items():

                        env = env or {}

                        extra = set(env) - {'conf', 'ctor', 'mirrors'}
                        if extra:
                            mesg = (f'getTestCluster() service {celltype} has invalid '
                                     f'envelope keys: {sorted(extra)}')
                            raise s_exc.BadArg(mesg=mesg)

                        conf = env.get('conf') or {}
                        mirrors = env.get('mirrors', 0)
                        ctor = env.get('ctor') or self.getTestSvcCtor(celltype)
    ...
    @contextlib.contextmanager
    def getLoggerStream(self, name, struct=True, level=logging.DEBUG):
        '''
        Capture log records from the named logger ( use '' for the root logger ).

        Args:
            name (str): The logger name to capture records from.
            struct (bool): Format the captured records as structured JSON.
            level (int): Only capture records at this level or above.
        '''
        stream = LoggerStream()
    ...
    @contextlib.asynccontextmanager
    async def getHttpSess(self, auth=None, port=None, headers=None):
        '''
        ...
        Args:
            ...
            headers (dict): Default headers to send on every request (e.g. ``{'X-API-KEY': ...}``).
        ...
        async with aiohttp.ClientSession(cookie_jar=jar, connector=conn, headers=headers) as sess:
    ...
    def _striptypes(self, model, ptyp, valu):
        '''
        Unwrap a stored, fully-typed value (as produced by Node.get()/pack()) to
        its bare value, dropping type names, so it can be compared against a bare
        expected value.
        '''
        if ptyp.ispoly:
            typename, subvalu = valu
            return self._striptypes(model, model.type(typename), subvalu)

        if ptyp.isarray:
            return tuple(self._striptypes(model, ptyp.arraytype, item) for item in valu)

        if isinstance(ptyp, s_types.Comp):
            return tuple(self._striptypes(model, model.type(ftypename), fvalu)
                         for ftypename, fvalu in valu)

        return valu

    def propeq(self, n, prop, valu, type=None, repr=False, msg=None):
        '''
        Assert a node property is equal to valu.

        For poly/array/comp props the comparison is by value: the stored value's
        type names are dropped (recursively) so the expected value may be given
        bare (e.g. a plain comp tuple).
        '''
        ...
        model = n.view.core.model

        if valu is not None and pval is not None and not repr:
            if ptyp.ispoly and type is not None:
                self.eq(pval[0], type, msg=msg)
                mtyp = model.type(type)
                self.eq(valu, self._striptypes(model, mtyp, pval[1]), msg=msg)
                return

            if ptyp.ispoly or ptyp.isarray or isinstance(ptyp, s_types.Comp):
                pval = self._striptypes(model, ptyp, pval)
    ...
    async def getTestCore(self, conf=None, dirn=None, prepkghook=None):
        ...
        async with SynTest.getTestCore(self, conf=None, dirn=None) as core:
            ...
            for pkgproto in self.pkgprotos:
                ...
                pkgdef = s_genpkg.loadPkgProto(pkgproto, readonly=True)
