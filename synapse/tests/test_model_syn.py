    ...
    async def test_syn_cmd_runts(self):
        ...
        async with self.getTestDmon() as dmon:
            ...
            async with self.getTestCore() as core:
                ...
                self.len(10, nodes)
                self.sorteq([
                    'deprvers', 'deprdate', 'deprmesg', 'ps.list', 'ps.kill', 'gen.it.av.scan.result',
                    'gen.ou.id.number', 'gen.ou.id.type', 'gen.ps.contact.email', 'gen.ou.org.hq',
                ], [k.ndef[1] for k in nodes])

                nodes = await core.nodes('syn:cmd:deprecated:version')
                self.len(8, nodes)
                self.sorteq([
                    'deprvers', 'ps.list', 'ps.kill', 'gen.it.av.scan.result',
                    'gen.ou.id.number', 'gen.ou.id.type', 'gen.ps.contact.email', 'gen.ou.org.hq',
                ], [k.ndef[1] for k in nodes])
                ...
                self.len(8, nodes)
                self.sorteq([
                    'deprmesg', 'ps.list', 'ps.kill', 'gen.it.av.scan.result',
                    'gen.ou.id.number', 'gen.ou.id.type', 'gen.ps.contact.email', 'gen.ou.org.hq',
                ], [k.ndef[1] for k in nodes])
