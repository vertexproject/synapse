...
class SynModelTest(s_t_utils.SynTest):
    ...
    async def test_model_syn_runts(self):
        ...
        async with self.getTestCore() as core:
            ...
            nodes = await core.eval('syn:type=testcomp').list()
            self.len(1, nodes)
            node = nodes[0]
            self.eq(('syn:type', 'testcomp'), node.ndef)
            self.eq({'fields': (('hehe', 'testint'), ('haha', 'testlower'))},
                    node.get('opts'))
            ...
            nodes = await core.eval('syn:form=testcomp').list()
            self.len(1, nodes)
            node = nodes[0]
            self.eq(('syn:form', 'testcomp'), node.ndef)
            self.nn(node.get('runt'))
            self.false(node.get('runt'))
            self.eq('testcomp', node.get('type'))
            self.eq('A fake comp type.', node.get('doc'))
            ...
            nodes = await core.eval('syn:prop="testtype10:intprop"').list()
            self.len(1, nodes)
            node = nodes[0]
            self.eq(('syn:prop', 'testtype10:intprop'), node.ndef)
            self.nn(node.get('ro'))
            self.false(node.get('ro'))
            self.nn(node.get('univ'))
            self.false(node.get('univ'))
            self.eq('int', node.get('type'))
            self.eq('testtype10', node.get('form'))
            ...
            nodes = await core.eval('syn:prop="testtype10"').list()
            self.len(1, nodes)
            node = nodes[0]
            self.eq(('syn:prop', 'testtype10'), node.ndef)
            self.eq('testtype10', node.get('form'))
            ...
            nodes = await core.eval('syn:prop="testcomp.created"').list()
            self.len(1, nodes)
            node = nodes[0]
            self.eq(('syn:prop', 'testcomp.created'), node.ndef)
            self.nn(node.get('univ'))
            self.false(node.get('univ'))
            ...
            nodes = await core.eval('syn:form=testcomp -> syn:prop:form').list()
            # form is a prop, two universal properties and two model secondary properties.
            self.len(1 + 2 + 2, nodes)
            ...
            async with await core.snap() as snap:
                node = await snap.addNode('test:edge', (('testint', 1234), ('teststr', '1234')))

            nodes = await core.eval('syn:form=testint -> test:edge:n1:form').list()
            self.len(1, nodes)
