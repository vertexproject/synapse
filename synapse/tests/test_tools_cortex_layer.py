...
class LayerTest(s_test.SynTest):
    ...
    async def test_tools_layer_dump_single_edit(self):
        ...
        async with self.getTestCore() as core:
            ...
            self.eq(soffs, core.getLayer(layr00iden).getEditIndx())
    ...
    async def test_tools_layer_dump_incremental(self):
        ...
        async with self.getTestCore() as core:
            ...
            with self.getTestDir() as dirn:
                ...
                self.eq(soffs + 4, core.getLayer(layr00iden).getEditIndx())
    ...
    async def test_tools_layer_dump_errors(self):
        ...
        async with self.getTestCore() as core:
            ...
            with self.getTestDir() as dirn:
                ...
                outp = self.getTestOutp()
                self.eq(1, await s_t_dump.main(argv, outp=outp))
                outp.expect('ERROR: No edits to export starting from offset (-1).')

                # An empty layer has no edits to export at any offset
                layr01 = await core.addLayer()
                layr01iden = layr01.get('iden')
                ...
                for offs in ('-1', '0'):
                    ...
                    outp = self.getTestOutp()
                    self.eq(1, await s_t_dump.main(argv, outp=outp))
                    outp.expect(f'ERROR: No edits to export starting from offset ({offs}).')
