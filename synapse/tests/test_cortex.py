...
class CortexTest(SynTest):
    ...
    def test_cortex_with_ingest(self):
        ...
        with s_cortex.openurl('ram:///') as core:
            ...
            core.regIngest('test', gest)
            self.eq(core._ingest_ref.get('test'), gest)

            core.regIngest('test2', gest2)

            # Dump data into the core an event at a time.
            core.ingestData('test', data1)
            ...
            core.ingestData('test2', data3)
            node = core.getTufoByProp('inet:fqdn', 'woot.com')
            self.true(isinstance(node, tuple))
            self.false(s_tufo.tagged(node, 'zoom.fooboat'))
            self.false(s_tufo.tagged(node, 'zoom.sewer'))

            # Ensure that unregister methods work
            core.unregIngest('test')
            self.eq(len(core._ingest_ref), 1)


    def test_cortex_with_ingest_bad(self):
        '''
        Ensure that unhappy paths work too
        '''
        data1 = {'foo':[ {'fqdn':'vertex.link','haha':['barbar','foofoo']} ] }
        ...
        with s_cortex.openurl('ram:///') as core:
            core.regIngest('test', gest)
            # Register a bad ingest handler
            core.regIngest('test2', gest2)

            # Register an existing handler without removal first.
            with self.raises(Exception) as cm:
                core.regIngest('test', gest)
            self.assertIn('Ingest handler test already registered', str(cm.exception))

            # Ingest with a unknown handler
            with self.raises(Exception) as cm:
                core.ingestData('foobarbaz', data1)
            self.assertIn('No handler registered for foobarbaz', str(cm.exception))

            # Duck-typing failure
            with self.raises(AttributeError) as cm:
                core.ingestData('test2', data1)
            self.assertIn("object has no attribute 'ingest'", str(cm.exception))

            # Unregister a nonexistent handler
            with self.raises(Exception) as cm:
                core.unregIngest('foobarbaz')
            self.assertIn('No handler registered for foobarbaz', str(cm.exception))
