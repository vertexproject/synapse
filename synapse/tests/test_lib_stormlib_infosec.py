...
class InfoSecTest(s_test.SynTest):
    ...
    async def test_stormlib_infosec_attack_flow(self):
        ...
        async with self.getTestCore() as core:
            ...
            self.len(1, nodes)
            self.propeq(nodes[0], 'name', 'lauren parker')
            self.propeq(nodes[0], 'email', 'lparker@mitre.org')
            contact = nodes[0]

            nodes = await core.nodes('it:mitre:attack:flow')
            self.len(1, nodes)
            self.propeq(nodes[0], 'name', 'CISA AA22-138B VMWare Workspace (Alt)')
            self.propeq(nodes[0], 'data', norm)
            self.propeq(nodes[0], 'created', s_time.parse('2023-02-21T14:51:27.768Z'))
            self.propeq(nodes[0], 'updated', s_time.parse('2023-03-10T19:54:29.098Z'))
            self.propeq(nodes[0], 'author:user', core.auth.rootuser.iden)
            self.propeq(nodes[0], 'author:contact', contact.repr())
