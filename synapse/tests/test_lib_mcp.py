...
class McpTest(s_tests.SynTest):
    ...
    async def test_mcp_lifecycle_and_sessions(self):
        ...
        async with self.getTestCell(TstMcpCell) as cell:
            ...
            url = f'https://localhost:{port}/api/v3/mcp'
    ...
    async def test_mcp_cortex_apikey_only(self):

        # the Cortex MCP endpoint (CortexMcp via ApiKeyOnlyMixin) authenticates with the
        # X-API-KEY header only -- session cookie / HTTP Basic auth are not honored.
        async with self.getTestCore() as core:
            ...
            await root.setPasswd('secret')
            apikey, _ = await core.addUserApiKey(root.iden, 'mcp')

            # HTTP Basic auth (which the base CellMcp would accept) is rejected here
            async with self.getHttpSess(auth=('root', 'secret'), port=port) as sess:
                async with sess.post(url, json=self._initBody()) as resp:
                    self.eq(resp.status, http.HTTPStatus.UNAUTHORIZED)

            # the same request authenticated with X-API-KEY succeeds
            async with self.getHttpSess(headers={'X-API-KEY': apikey}) as sess:
                status, sid, _ = await self._initOnly(sess, url)
                self.eq(status, http.HTTPStatus.OK)
                self.nn(sid)
    ...
    async def test_mcp_cortex_views(self):
        ...
        async with self.getTestCore() as core:
            ...
            lowkey, _ = await core.addUserApiKey(lowuser.iden, 'mcp')
            ...
            async with self.getHttpSess(headers={'X-API-KEY': apikey}) as sess:
                ...
                # view_merge schedules a background merge of a fork's edits into its parent
                ...
                mergeview = core.getView(mergeiden)
                ...
                # the background merge removes the fork (and its top layer) when it completes
                self.true(await mergeview.waitfini(timeout=12))
            ...
            async with self.getHttpSess(headers={'X-API-KEY': lowkey}) as sess:
    ...
    async def test_mcp_cortex_remote(self):
        ...
        async with self.getTestCore() as core:
            ...
            async with core.getLocalProxy() as prox:
                ...
                host, port = await core.addHttpsPort(0, host='127.0.0.1')
                url = f'https://localhost:{port}/api/v3/mcpremote'
                ...
                async with self.getHttpSess(headers={'X-API-KEY': apikey}) as sess:
                    ...
                    status, data = await self._tool(sess, url, sid, 'model_find', {'pattern': 'inet:ip'})
                    self.isin('inet:ip', data['result']['structuredContent']['forms'])
                    ...
                    await self._tool(sess, url, sid, 'storm', {'query': '[ inet:ip=1.2.3.4 ]'})
                    ...
                    self.isin('inet:ip', data['result']['completion']['values'])
    ...
    async def test_mcp_cortex_tools(self):
        ...
        async with self.getTestCore() as core:
            ...
            async with self.getHttpSess(headers={'X-API-KEY': apikey}) as sess:
                ...
                status, data = await self._tool(sess, url, sid, 'storm_validate', {'query': 'inet:ip=1.2.3.4'})
                ...
                self.isin('inet:ip', res['types'])
                self.isin('inet:ip', res['forms'])

                # a doc-only match (no name contains "ipv4 or ipv6") still finds inet:ip by its doc
                status, data = await self._tool(sess, url, sid, 'model_find', {'pattern': '(?i)IPv4 or IPv6'})
                self.isin('inet:ip', data['result']['structuredContent']['types'])
                ...
                self.isin('ip', res['forms']['inet:dns:a']['props'])
                ...
                status, data = await self._tool(sess, url, sid, 'storm', {'query': '[ inet:ip=1.2.3.4 ]'})
                res = data['result']['structuredContent']
                ...
                status, data = await self._rpc(sess, url, sid, 'resources/read',
                                               params={'uri': 'syn://model/form/inet:ip'})
                self.eq('inet:ip', s_json.loads(data['result']['contents'][0]['text'])['name'])
                ...
                status, data = await self._rpc(sess, url, sid, 'completion/complete',
                                               params={'ref': {'type': 'ref/resource', 'uri': 'syn://model/form/{name}'},
                                                       'argument': {'name': 'name', 'value': 'inet:i'}})
                self.isin('inet:ip', data['result']['completion']['values'])
