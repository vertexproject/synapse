...
class LayerTest(s_t_utils.SynTest):
    ...
    async def test_layer_edit_batch(self):
        ...
        async with self.getTestCore() as core:
            ...
            base = core.getLayer()
            forkiden = await core.callStorm('return($lib.view.get().fork().layers.0.iden)')
            fork = core.getLayer(forkiden)

            form = core.model.form('test:int')
            stortype = form.type.stortype

            buid0 = s_common.buid(('test:int', 10))
            buid1 = s_common.buid(('test:int', 20))

            # edits for the same (layer, buid) coalesce into one nodeedit
            batch = s_layer.LayerEditBatch(core, meta)

            batch.addEdit(base.iden, buid0, 'test:int', (
                (s_layer.EDIT_NODE_ADD, (10, stortype), ()),
            ))
            batch.addEdit(base.iden, buid0, 'test:int', (
                (s_layer.EDIT_TAG_SET, ('foo', (None, None), None), ()),
            ))
            batch.addEdit(fork.iden, buid1, 'test:int', (
                (s_layer.EDIT_NODE_ADD, (20, stortype), ()),
            ))

            self.eq(3, batch.size())
            self.len(1, batch.nodeedits[base.iden])

            applied = await batch.flush()

            # one entry per layer, and the batch is reset
            self.sorteq([base.iden, fork.iden], list(applied.keys()))
            self.len(1, applied[base.iden])
            self.eq(0, batch.size())
            self.eq({}, batch.nodeedits)

            # the coalesced nodeedit applied both edits to the one buid
            self.eq(buid0, applied[base.iden][0][0])
            etyps = [edit[0] for edit in applied[base.iden][0][2]]
            self.isin(s_layer.EDIT_NODE_ADD, etyps)
            self.isin(s_layer.EDIT_TAG_SET, etyps)

            # the node landed in the layer it was addressed to
            self.len(1, await core.nodes('test:int=10 +#foo'))
            self.len(0, await core.nodes('test:int=20'))

            # re-applying the same edits is a no-op, so nothing comes back
            batch.addEdit(base.iden, buid0, 'test:int', (
                (s_layer.EDIT_NODE_ADD, (10, stortype), ()),
            ))
            self.eq({}, await batch.flush())

            # nolift=True uses storNodeEditsNoLift() and returns nothing
            batch = s_layer.LayerEditBatch(core, meta, nolift=True)
            batch.addEdit(base.iden, s_common.buid(('test:int', 30)), 'test:int', (
                (s_layer.EDIT_NODE_ADD, (30, stortype), ()),
            ))
            self.eq({}, await batch.flush())
            self.len(1, await core.nodes('test:int=30'))

            # chunk triggers a flush once the edit count is reached
            batch = s_layer.LayerEditBatch(core, meta, chunk=2)
            for valu in (40, 41):
                await batch.addNodeEdit(base.iden, (
                    s_common.buid(('test:int', valu)), 'test:int', (
                        (s_layer.EDIT_NODE_ADD, (valu, stortype), ()),
                    ),
                ))

            self.eq(0, batch.size())
            self.len(2, await core.nodes('test:int=40 test:int=41'))

            # an unknown layer iden is skipped
            batch = s_layer.LayerEditBatch(core, meta)
            batch.addEdit(s_common.guid(), s_common.buid(('test:int', 50)), 'test:int', (
                (s_layer.EDIT_NODE_ADD, (50, stortype), ()),
            ))
            self.eq({}, await batch.flush())
            self.len(0, await core.nodes('test:int=50'))
