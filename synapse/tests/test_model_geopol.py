...
class GeoPolModelTest(s_t_utils.SynTest):
    ...
    async def test_model_geopol_election(self):
        async with self.getTestCore() as core:
            ...
            self.len(1, await core.nodes('pol:candidacy -> pol:race'))
            self.len(1, await core.nodes('pol:candidacy -> ou:org +:name=vertex'))
            self.len(1, await core.nodes('pol:candidacy -> entity:contact +:name=whippit'))
            self.len(1, await core.nodes('pol:candidacy -> entity:campaign +:name=whippit4prez'))
