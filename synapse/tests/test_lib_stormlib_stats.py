...
class StatsTest(s_test.SynTest):
    ...
    async def test_stormlib_stats_countby(self):
        ...
        async with self.getTestCore() as core:
            ...
            msgs = await core.stormlist('inet:ip | stats.countby :asn --bar-width 10')
            self.stormIsInPrint(chartwidth, msgs)

            msgs = await core.stormlist('inet:ip | stats.countby :asn --bar-width 10 --by-name')
            self.stormIsInPrint(chartwidth_byname, msgs)

            msgs = await core.stormlist('inet:ip | stats.countby :asn --label-max-width 1')
            self.stormIsInPrint(chartlabelwidth, msgs)

            msgs = await core.stormlist('inet:ip | stats.countby :asn --label-max-width 1 --by-name')
