...
$request.reply(206, headers=$headers, body=({"no":"body"}))
            ...
            async with self.getHttpSess(port=hport, headers={'X-API-KEY': rootkey}) as sess:  # type: aiohttp.ClientSession
                ...
                emsg = f'Error executing Extended HTTP API {test04}: BadArg Response.reply() has already been called.'
                with self.getLoggerStream('synapse.lib.httpapi') as stream:
                    ...
                    await stream.expect(emsg, timeout=6)
...
for $i in $values {
    ...
    async def test_libcortex_httpapi_headers_params(self):
        ...
        async with self.getTestCore() as core:
            ...
            $api.methods.post = ${
                if $request.headers.popmethod {
                    $lib.dict.pop($request.headers, "Secret-Key")
                } elif $request.headers.dictmethod {
            ...
            async with self.getHttpSess(port=hport, headers={'X-API-KEY': rootkey}) as sess:
                ...
                resp = await sess.post(f'https://localhost:{hport}/api/ext/testpath', headers={'popmethod': '1'})
                self.eq(resp.status, http.HTTPStatus.INTERNAL_SERVER_ERROR)
                data = await resp.json()
                self.eq(data.get('code'), 'StormRuntimeError')
                self.eq(data.get('mesg'), 'http:api:request:headers may not be modified by the runtime.')
        ...
        async with self.getTestCore() as core:
            ...
            async with self.getHttpSess(port=hport, headers={'X-API-KEY': rootkey}) as sess:
                ...
                with self.getLoggerStream('synapse.lib.httpapi') as stream:
                    ...
                    await stream.expect(f'Extended HTTP API {iden03} tried to set code after sending body.', timeout=6)
                    self.eq(resp.status, http.HTTPStatus.CREATED)
                    self.eq(await resp.read(), b'text')

                with self.getLoggerStream('synapse.lib.httpapi') as stream:
                    resp = await sess.get(f'https://localhost:{hport}/api/ext/bad04')
                    await stream.expect(f'Extended HTTP API {iden04} tried to set headers after sending body.', timeout=6)
                    self.eq(resp.status, http.HTTPStatus.OK)
                    self.eq(await resp.read(), b'text')
                ...
                with self.getLoggerStream('synapse.lib.httpapi') as stream:
                    ...
                    await stream.expect(f'Error executing Extended HTTP API {iden06}: BadTypeValu', timeout=6)
                    self.eq(resp.status, http.HTTPStatus.CREATED)
                    self.eq(await resp.json(), {})

                with self.getLoggerStream('synapse.lib.httpapi') as stream:
                    resp = await sess.get(f'https://localhost:{hport}/api/ext/bad07')
                    await stream.expect(f'Error executing Extended HTTP API {iden07}: StormRuntimeError', timeout=6)
                    self.eq(resp.status, http.HTTPStatus.INTERNAL_SERVER_ERROR)
                    data = await resp.json()
                    self.eq(data.get('code'), 'StormRuntimeError')
    ...
    async def test_libcortex_nids(self):
        ...
        async with self.getTestCore() as core:
            ...
            opts = {'vars': {'nid': nid}}
            rndef = await core.callStorm('return($lib.cortex.getNodeByNid($nid).ndef)', opts=opts)
            self.eq(rndef, ('test:str', 'foo'))

            opts = {'vars': {'nid': nid}}
            rndef = await core.callStorm('return($lib.cortex.getNdefByNid($nid))', opts=opts)
            self.eq(rndef, ('test:str', 'foo'))

            opts = {'vars': {'ndef': ndef}}
            rnid = await core.callStorm('return($lib.cortex.getNidByNdef($ndef))', opts=opts)
            self.eq(rnid, nid)
