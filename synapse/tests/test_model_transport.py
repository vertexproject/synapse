...
class TransportTest(s_test.SynTest):
    ...
    async def test_model_transport(self):
        ...
        async with self.getTestCore() as core:
            ...
            flightnum = (await core.nodes('[ transport:air:flightnum="ua 2437" :carrier=* as ou:org :from:port=IAD :to:port=LAS :stops=(IAD,VISI,LAS) ]'))[0]

            flight = (await core.nodes('''
                [ transport:air:flight=*
                    :num=UA2437
                    :scheduled:period=(20200202, 20200203)
                    :period=(2020020202, 202002020302)
                ]'''))[0]

            self.propeq(flight, 'num', 'ua2437')
            self.propeq(flight, 'scheduled:period', (1580601600000000, 1580688000000000, 86400000000))
            self.propeq(flight, 'period', (1580608800000000, 1580612520000000, 3720000000))
            ...
            seatelem = (await core.nodes('''[
                 transport:sea:telem=*
                    :time=20200202
                    :vessel=* as transport:sea:vessel
                    :place:loc=us
                    :place:latlong=(20.22, 80.1111)
                    :place:latlong:accuracy=10m
                    :place=* as geo:place
            ...
            airport = (await core.nodes('transport:air:port=VISI [:name="Visi Airport" :place=* as geo:place]'))[0]
            ...
            nodes = await core.nodes('''
                ...
                [ transport:land:registration=$regid
                    ...
                    :vehicle={[ transport:land:vehicle=*
                        :serial=V-31337
                        :model=elise
                        :registration=$regid as transport:land:registration
                        :type=car
                    ]}
        ...
        async with self.getTestCore() as core:
            nodes = await core.nodes('''[
                transport:rail:train=*
                    ...
                    :period=(202501171030, 202501171330)
                    :scheduled:period=(202501171030, 202501171330)
                    ...
                    :vehicle={[
                        transport:rail:consist=*
                            ...
                            :cars={[
                                ...
                                :max:cargo:volume=1000l
                            ]}
                    ]}
                    :operator={[ entity:contact=* :name="visi" ]}
            ]''')

            self.propeq(nodes[0], 'period', (1737109800000000, 1737120600000000, 10800000000))
            self.propeq(nodes[0], 'scheduled:period', (1737109800000000, 1737120600000000, 10800000000))
            ...
            self.true(core.model.form('transport:air:flight').implements('meta:usable'))
            self.true(core.model.form('transport:land:drive').implements('meta:usable'))
            self.true(core.model.form('transport:rail:train').implements('meta:usable'))
            self.true(core.model.form('transport:occupant').implements('entity:activity'))
