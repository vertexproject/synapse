...
class PsModelTest(s_test.SynTest):
    ...
    def test_ps_altname(self):

        with self.getTestCore() as core:

            with core.snap() as snap:

                node = snap.addNode('ps:person', '*')

                node.set('name', 'foo')
                node.set('name', 'bar')

                nodes = list(snap.getNodesBy('ps:altname:person', node.ndef[1]))
                self.len(2, nodes)
                self.eq(list(sorted([n.get('name') for n in nodes])), ('bar', 'foo'))
