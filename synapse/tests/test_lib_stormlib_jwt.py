...
class StormLibJwtJwksTest(s_test.SynTest):
    ...
    async def test_stormlib_jwt_jwks_uri(self):
        ...
        async with self.getTestCore() as core:
            ...
            base = {'t': tok, 'k': None, 'url': url, 'ssl': ssl}

            # https is required
            httpurl = f'http://localhost:{port}/jwks'
            self.false((await core.callStorm(
                'return($lib.crypto.jwt.verify($t, $k, ("RS256",), jwks_uri=$url, ssl_opts=$ssl))',
            ...
            ok, sub = await core.callStorm('''
                ($ok, $v) = $lib.crypto.jwt.verify($t, $k, ("RS256",),
                    jwks_uri=$url, ssl_opts=$ssl, allowinternal=(true))
            ...
            self.true((await core.callStorm(
                'return($lib.crypto.jwt.verify($t, $k, ("RS256",), jwks_uri=$url, ssl_opts=$ssl, allowinternal=(true)))',
                opts={'vars': base}))[0])
            self.eq(state['hits'], 1)
        ...
        async with self.getTestCore() as core:
            ...
            q = '''
                return($lib.crypto.jwt.verify($t, $k, ("RS256",),
                    jwks_uri=$url, ssl_opts=$ssl, allowinternal=(true)))
            '''
            self.true((await core.callStorm(q, opts={'vars': {'t': tok1, 'k': None, 'url': url, 'ssl': ssl}}))[0])
            self.eq(state['hits'], 1)

            # rotate: the endpoint now serves both keys. An unknown kid forces one refresh.
            state['body'] = s_json.dumps({'keys': [jwk1, jwk2]})
            self.true((await core.callStorm(q, opts={'vars': {'t': tok2, 'k': None, 'url': url, 'ssl': ssl}}))[0])
    ...
    async def test_stormlib_jwt_jwks_uri_stale(self):
        ...
        async with self.getTestCore() as core:
            ...
            ok = (await core.callStorm('''
                return($lib.crypto.jwt.verify($t, $k, ("RS256",),
                    jwks_uri=$url, ssl_opts=$ssl, allowinternal=(true)))
            ''', opts={'vars': {'t': tok, 'k': None, 'url': url, 'ssl': ssl}}))[0]
            self.true(ok)
            self.eq(state['hits'], 1)
        ...
        async with self.getTestCore() as core:
            ...
            def check(path, body, code=200):
                ...
                return core.callStorm('''
                    return($lib.crypto.jwt.verify($t, $k, ("RS256",),
                        jwks_uri=$url, ssl_opts=$ssl, allowinternal=(true)))
                ''', opts={'vars': {'t': tok, 'k': None, 'url': url, 'ssl': ssl}})
    ...
    async def test_stormlib_jwt_jwks_uri_norefetch(self):
        ...
        async with self.getTestCore() as core:
            ...
            self.false((await core.callStorm('''
                return($lib.crypto.jwt.verify($t, $k, ("RS256",), jwks_uri=$url, ssl_opts=$ssl, allowinternal=(true)))
            ''', opts={'vars': {'t': toknokid, 'k': None, 'url': urlA, 'ssl': ssl}}))[0])
            ...
            self.false((await core.callStorm('''
                return($lib.crypto.jwt.verify($t, $k, ("ES256",), jwks_uri=$url, ssl_opts=$ssl, allowinternal=(true)))
            ''', opts={'vars': {'t': estok, 'k': None, 'url': urlB, 'ssl': ssl}}))[0])
