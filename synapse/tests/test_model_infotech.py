...
class InfotechModelTest(s_t_utils.SynTest):
    ...
    async def test_infotech_cpes(self):
        ...
        async with self.getTestCore() as core:
            ...
            self.eq(nodes[0].get('v2_2'), 'cpe:/h:d-link:dir-850l')
            ...
            nodes = await core.nodes('[ it:sec:cpe="cpe:2.3:o:*:linux:3.10.0\-229.14.1.el7.centos.plus.x86_64:*:*:*:*:*:x86_64:*" ]')
            self.len(1, nodes)
            self.eq(nodes[0].ndef, ('it:sec:cpe', 'cpe:2.3:o:*:linux:3.10.0\-229.14.1.el7.centos.plus.x86_64:*:*:*:*:*:x86_64:*'))
            self.eq(nodes[0].get('part'), 'o')
            self.eq(nodes[0].get('product'), 'linux')
            self.eq(nodes[0].get('target_hw'), 'x86_64')
            self.eq(nodes[0].get('version'), '3.10.0-229.14.1.el7.centos.plus.x86_64')
            self.eq(nodes[0].get('v2_2'), 'cpe:/o::linux:3.10.0-229.14.1.el7.centos.plus.x86_64::~~~~x86_64~')

            nodes = await core.nodes('[ it:sec:cpe="cpe:2.3:o:cisco:ios:12.1\(22\)ea1a:*:*:*:*:*:*:*" ]')
            self.len(1, nodes)
            self.eq(nodes[0].ndef, ('it:sec:cpe', 'cpe:2.3:o:cisco:ios:12.1\(22\)ea1a:*:*:*:*:*:*:*'))
            self.eq(nodes[0].get('part'), 'o')
