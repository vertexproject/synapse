...
class WebAppTest(SynTest):
    ...
    def test_webapp_publish(self):
        ...
        resp = requests.get('http://127.0.0.1:%d/v1/addup/30?y=40' % port).json()

        self.assertEqual( resp.get('ret'), 70 )
        self.assertEqual( resp.get('status'), 'ok' )

        resp = requests.get('http://127.0.0.1:%d/v1/addup/20' % port).json()

        self.assertEqual( resp.get('ret'), 20 )
        self.assertEqual( resp.get('status'), 'ok' )

        resp = requests.get('http://127.0.0.1:%d/v1/horked' % port).json()
