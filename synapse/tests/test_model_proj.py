...
class ProjModelTest(s_test.SynTest):
    ...
    async def test_model_proj(self):
        ...
        async with self.getTestCore() as core:
            ...
            opts = {'vars': {'proj': proj}}
            self.len(1, await core.nodes('yield $lib.projects.add($proj)', opts=opts))
            self.len(1, await core.nodes('yield $lib.projects.get($proj).epics.add(epic)', opts=opts))
            self.len(1, await core.nodes('yield $lib.projects.get($proj).sprints.add(spri)', opts=opts))
            self.len(1, await core.nodes('yield $lib.projects.get($proj).tickets.add(tick)', opts=opts))
            self.len(1, await core.nodes('yield $lib.projects.get($proj).tickets.get(tick).comments.add(comm)', opts=opts))
