...
```mdstorm
ou:org:name=vertex [ :names=('The Vertex Project',) ]
```

Set the `:names` property of an organization (`ou:org`) node to contain multiple variations of the organization name:

```mdstorm
ou:org:name=vertex [ :names=('The Vertex Project', 'The Vertex Project, LLC') ]
```
...
```mdstorm --hide
[ ou:org=( { "name": "Monty Python" } ) :names+="Monty Python's Flying Circus" ]
```

```mdstorm
ou:org:name='Monty Python' [ :names+='The Spanish Inquisition' ]
```
...
```mdstorm --hide
[ entity:contact=( { "name": "ron the cat", "type": "vertex.employee" } ) :emails+=ron@vertex.link ]
```

```mdstorm
entity:contact:name='ron the cat' [ :emails?+='ron[at]protonmail.com' ]
```
...
```mdstorm
ou:org:names=('The Vertex Project', 'The Vertex Project, LLC')
```
...
```mdstorm
ou:org:names*[='the vertex project']
```
...
```mdstorm --hide
[ crypto:x509:cert=( { "sha256": "6b60c1c833979494caff32bf02391793ac85f533516367f12a1cea857bbacba7", "issuer": "CN=Microsoft Certificate Authority" } ) :identities:fqdns=(microsoft.com, office365.com) ]
```
...
```mdstorm
doc:report:publisher:name=proofpoint +:topics*[^=cyber]
```
...
```mdstorm --hide
[ ( ou:org=( { "name": "ministry of public security" } ) :names+=mps ) ( ou:org=( { "name": "valisministeerium" } ) :names+='ministry of foreign affairs of estonia' ) ]
```
...
```mdstorm --hide
$file={ [ file:bytes=( { "sha256": "000185a17254cd8863208d3828366ec25ddd01596f18e57301355d4a33eac242" } ) ] } [ it:sec:c2:config=( { "file": $file, "family": "redtree" } ) ]
```

```mdstorm
it:sec:c2:config:family=redtree [ :connect:delay=00:17:00 ]
```

Set the duration of a compromise (`risk:compromise` node) to six days, six hours, 46 minutes, and 23 seconds:

```mdstorm --hide
[ risk:compromise=( { "name": "example compromise", "reporter:name": "vertex" } ) ]
```

```mdstorm
risk:compromise:name='example compromise' [ :period.duration='6D 06:46:23' ]
```

Or:

```mdstorm
risk:compromise:name='example compromise' [ :period.duration=(542783000000) ]
```
...
```mdstorm --hide
entity:name=vertex -> ou:org | delnode
```

```mdstorm --hide
[ ou:org=* ou:org=* :name='the vertex project' :websites+=https://vertex.link/ ]
```
...
```mdstorm --hide
ou:org:name='the vertex project' [ :dns:mx=(vertex.link, vtx.lk) ]
```
...
```mdstorm --hide
[ meta:note=(asnote,) :text="example note" ]
```

```mdstorm
meta:note:text="example note" [ :creator=( { "$as": "ps:person", "name": "alice liddell" } ) ]
```
...
```mdstorm --hide
ou:org:name='vertex project' | delnode
```

```mdstorm
[ ou:org=( { "name": "vertex project", "$props": { "desc": "a great org", "phone": "15551234567" } } ) ]
```
...
```mdstorm --hide
biz:listing:name='widget' | delnode
```

```mdstorm
[ biz:listing=( { "name": "widget", "price": "10", "price.currency": "usd" } ) ]
```

*Example -- econ:price virtual key in \`\`\$props\`\`*:

```mdstorm --hide
biz:listing:name='widget2' | delnode
```

```mdstorm
[ biz:listing=( { "name": "widget2", "price": "5", "$props": { "price.currency": "usd" } } ) ]
```
...
```mdstorm
[ ou:org=( { "name": "vertex project", "$try": true, "$props": { "phone": "not-a-phone-number", "desc": "fine value" } } ) ]
```
...
```mdstorm --hide
ou:org:name='salty org' | delnode
```

```mdstorm
[ ou:org=( { "name": "salty org", "$salt": "source-a" } ) ]
```
...
```mdstorm
[ ou:org=( { "name": "example org" } ) ]
```

```mdstorm
[ ou:org=( { "name": "example org", "$unsets": ["desc"] } ) ]
```
...
```mdstorm
[ ou:org=('the vertex project', 'https://vertex.link/') ]
```
...
```mdstorm
[ ou:org=('the vertex project', 'https://vertex.link/') :name='the vertex project' :websites+=https://vertex.link/ ]
```
...
```mdstorm
$name='the vertex project' $url=https://vertex.link/ [ ou:org=($name, $url) ]
```

```mdstorm
$guid=$lib.guid('the vertex project', https://vertex.link/) [ ou:org=$guid ]
```

```mdstorm
$name='the vertex project' $url=https://vertex.link/ $guid=$lib.guid($name, $url) [ ou:org=$guid ]
```

```mdstorm
$name='the vertex project' $url=https://vertex.link/ [ ou:org=$lib.guid($name, $url) ]
```
...
```mdstorm --hide
[ inet:dns:request=( { "time": "2020/04/30 09:30:33.000", "query:name": "pop.seznam.cz" } ) ]
```

```mdstorm
inet:dns:request:query:name=pop.seznam.cz
```
...
```mdstorm --hide
[ entity:contact=( { "name": "ozzie the pony", "type": "vertex.employee" } ) :org:name=vertex ]
```

```mdstorm
entity:contact^=92a43a
```
...
```mdstorm --hide
[ ou:org=( { "name": "the vertex project" } ) ( entity:contact=( { "name": "ron the cat", "type": "vertex.employee" } ) :title='cattribution analyst' ) ]
```
...
```mdstorm --hide
[ inet:fqdn=ayuisyahooapis.com inet:fqdn=micyuisyahooapis.com inet:fqdn=usyahooapis.com ]
```

```mdstorm
inet:fqdn='*yahooapis.com'
```

Lift all domains ending with `s.wordpress.com`:

```mdstorm --hide
[ inet:fqdn=cats.wordpress.com inet:fqdn=dogs.wordpress.com inet:fqdn=www.sss.wordpress.com ]
```
...
```mdstorm --hide
[ inet:dns:a=(woot.com, 1.2.3.4) inet:dns:a=(woot.museum, 5.6.7.8) inet:dns:a=(woot.link, 7.7.7.7) ] +:fqdn='*.museum'
```
...
```mdstorm --hide
[ inet:ip=2606:4700:3031::6815:1c72 inet:ip=2606:4700:3031::6815:1c76 inet:ip=2606:4700:3031::6815:1c77 ]
```

```mdstorm
inet:ip=2606:4700:3031::6815:1c70-2606:4700:3031::6815:1c79
```
...
```mdstorm
inet:dns:a:fqdn=woot.com +:ip=172.16.0.0/12
```
...
```mdstorm --hide
[ biz:listing=( { "name": "widgets for sale", "price": "10" } ) :price.currency=usd ]
```

```mdstorm
biz:listing:name='widgets for sale' [ :count:total=42 ]
```

Use a hex value to set the `:ip:proto` property for an `inet:flow` node to 6:

```mdstorm --hide
[ inet:flow=( { "period": "2026/04/27 09:37:42", "server": "tcp://142.118.95.50:8888" } ) ]
```

```mdstorm
inet:flow:server.ip=142.118.95.50 [ :ip:proto=0x06 ]
```
...
```mdstorm
file:mime:rar:entry [ :extra:posix:perms=0o755 ]
```
...
```mdstorm --hide
[ risk:alert=( { "name": "outbound traffic to SOC-reported IP", "created": "2023/08/01 09:00:00", "priority": "9" } ) ]
```

```mdstorm
risk:alert:priority>5
```
...
```mdstorm --hide
[ ( ou:org=( { "name": "republic of turkey ministry of foreign affairs" } ) :place:loc=tr.ankara ) ( ou:org=( { "name": "adeo it consulting services" } ) :place:loc=tr.istanbul ) ]
```
...
```mdstorm --hide
[ ( inet:ip=149.248.52.240 :place:loc=ca.on ) ( inet:ip=49.51.12.195 :place:loc=ca.on.barrie ) ( inet:ip=199.201.123.200 :place:loc=ca.on.keswick ) ]
```

```mdstorm
inet:ip:place:loc^=ca.on
```
...
```mdstorm --hide
[ ou:org=( { "name": "acme construction" } ) ou:org=( { "name": "acme practical joke products" } ) ]
```
...
```mdstorm --hide
[ syn:tag=cno.threat.t27 syn:tag=cno.mal.redtree ]
```
...
```mdstorm --hide
[ ( syn:tag=rep.sophos.cobalt_shadow :title="COBALT SHADOW (Sophos)" ) ( syn:tag=rep.sophos.copper_fieldstone :title="COPPER FIELDSTONE (Sophos)" ) ( syn:tag=rep.sophos.iron_tilden :title="IRON TILDEN (Sophos)" ) ]
```

```mdstorm
syn:tag^=rep.sophos.co
```
...
```mdstorm
entity:goal:type:taxonomy=financial_gain $lib.print($node.repr()) $lib.print($node.value)
```
...
```mdstorm --hide
[ ind:industry=( { "name": "media", "type": "media" } ) ind:industry=( { "name": "journalism and news media", "type": "media.journalism" } ) ind:industry=( { "name": "journalists and reporters", "type": "media.journalism.reporters" } ) ind:industry=( { "name": "publishing", "type": "media.publishing" } ) ind:industry=( { "name": "print media", "type": "media.publishing.print" } ) ind:industry=( { "name": "entertainment", "type": "media.entertainment" } ) ]
```

```mdstorm
ind:industry:type^=media
```

Lift all industries (`ind:industry` nodes) within the "media" category / taxonomy whose subcategory starts with "j":

```mdstorm
ind:industry:type^=media.j
```
...
```mdstorm
[ inet:ip=1.2.3.4 +#cno.infra.dns.sink.hole=(2017/06/13, '?') ]
```

- Set the observed time window using a time zone offset:

```mdstorm
[ inet:ip=5.6.7.8 +#cno.infra.dns.sink.hole=(2017/06/13 09:46+04:00, '?') ]
```
...
```mdstorm --hide
[ inet:dns:request=( { "query:name": "example.com" } ) ]
```

```mdstorm
inet:dns:request:query:name=example.com [ :time=2021/01/23 ]
```
...
```mdstorm --hide
[ inet:dns:request=( { "query:name": "hurr.com", "time": "2018/06/28 17:43" } ) inet:dns:request=( { "query:name": "derp.net", "time": "2015/12/14 19:22" } ) ]
```
...
```mdstorm
file:mime:pe:compiled*range=(2021/01/01, now)
```
