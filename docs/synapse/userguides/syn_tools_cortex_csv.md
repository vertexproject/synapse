<a id="syn-tools-cortex-csv"></a>


# cortex.csv

The Synapse `cortex.csv` command can be used to ingest structured data from a comma-separated values (CSV) file to create nodes in a Cortex. `cortex.csv` is useful for bulk-loading CSV-formatted data without the need to develop custom ingest code. (For other data formats such as JSON, yaml, or msgpack, see [cortex.feed](syn_tools_cortex_feed.md#syn-tools-cortex-feed).)

The `--export` option can be used to export a set of data from a Cortex into a CSV file.

Storm queries are used both to ingest and export data using `cortex.csv`. Users should be familiar with the Storm query language ([Storm Reference - Introduction](storm_ref_intro.md#storm-ref-intro) et al.) and the Synapse data model ([Data Model Objects](data_model.md#data-model-terms) et al.) in order to use `cortex.csv` effectively.

The Storm syntax used with `cortex.csv` makes use of a few more advanced Storm concepts such as variables, methods, libraries, and some programming flow control concepts (e.g., for loops and switch statements). However, the examples below should be fairly self-explanatory. In other words, users do **not** need to understand in detail how those concepts work in order to create basic `stormfile` queries and start loading data using `cortex.csv`.
...
- [Storm Reference - Subqueries](storm_ref_subquery.md#storm-ref-subquery)
...
- `LOGFILE` is the optional name / path to log Storm events associated with running the `cortex.csv` command as a JSONL file. Messages are appended to this file when they are written to them.
...
> [!NOTE]
> This tool was previously run using `synapse.tools.csvtool`, which was removed in Synapse 3.0.0. See [CLI Tool Changes](../300_changes/devops-cli-tools.md#vtx_300_devops-cli-tools).

<a id="cortex-csv-examples-ingest"></a>


## Ingest Examples - Overview

The key components for using the `cortex.csv` command are the CSV file itself (`csvfile`) and the file containing the Storm query (`stormfile`) used to ingest the data.
...
> [!NOTE]
> Because the file contains a header row, we need to use the `--csv-header` option to tell `cortex.csv` to skip the first row when ingesting data.
...
- Data in the CSV file that does not pass [Type](data_model.md#data-type) validation on node creation (i.e., bad or incorrect data, such as an IP address in an FQDN column).
...
We can now issue Storm commands to interact with and validate the data (i.e., did `cortex.csv` create the expected number of nodes, were the properties set correctly, etc.)
...
<a id="csvtool-examples-export"></a>
...
For this example, we will export the data we imported in [Ingest Example 2](syn_tools_cortex_csv.md#ingest-2). For this simple example, we want to export the set of malicious indicators associated with the Vicious Wombat threat group.
...
[$node](storm_adv_vars.md#vars-node-node) is a built-in Storm variable that represents the **current node** passing through the Storm pipeline. `$node` supports a number of methods ([Storm Reference - Advanced - Methods](storm_adv_methods.md#storm-adv-methods)) that allow Storm to access various attributes of the current node. In this case:

- The [$node.form](storm_adv_methods.md#meth-node-form) method will access (return) the current node's form.
- The [$node.value](storm_adv_methods.md#meth-node-value) method will access (return) the current node's primary property value.
...
For this example, we will export the DNS A records we imported in [Ingest Example 1](syn_tools_cortex_csv.md#ingest-1). We will create a CSV file that matches the format of our original ingest file, with columns for domain, IP, and first / last resolution times.
...
Synapse stores IP addresses as tuples of integers, so specifying `:ip` for our output definition gives us the raw value for that property. If we want the human-readable value, we need to use the human-friendly representation ([Repr](../glossary.md#gloss-repr)) of the value. We can do this using the [$node.repr()](storm_adv_methods.md#meth-node-repr) method to tell Storm to obtain and use the repr value of a node instead of its raw value ([$node.value](storm_adv_methods.md#meth-node-value)).
...
>
> Subqueries ([Storm Reference - Subqueries](storm_ref_subquery.md#storm-ref-subquery)) or various flow control processes ([Storm Reference - Advanced - Control Flow](storm_adv_control.md#storm-adv-control)) can be used to conditionally account for the presence or absence of data for a given node.
