...
```mdstorm --hide
[ inet:whois:record=( { "created": "2014/08/15", "fqdn": "vertex.link" } ) inet:whois:record=( { "created": "2010/09/03", "fqdn": "vertex.link" } ) ]
```
...
```mdstorm --hide
[ inet:email=info@vertex.link inet:email=visi@vertex.link inet:email=support@vertex.link ]
```

```mdstorm --hide
inet:email:fqdn=vertex.link | count
```
...
```mdstorm
$ip=1.2.3.4 $port=22 inet:client=`tcp://{$ip}:{$port}`
```

- Use node properties in a string:

```mdstorm --hide
[ inet:ip=8.8.8.8 :asn=15169 :seen=('2024/12/09 03:22:57', '2026/07/11 18:06:49') +#mytag=('2025/03/12', '2025/09/27') ]
```

```mdstorm
inet:ip=8.8.8.8 $lib.print(`IP {$node.repr()}: asn={:asn} :seen={:seen} mytag={#mytag}`) | spin
```

Backtick format strings may also span multiple lines, which will include the newlines when displayed:

```mdstorm
inet:ip=8.8.8.8 $lib.print(`\nIP {$node.repr()}:\nasn={:asn}\nseen={:seen}\nmytag={#mytag}`) | spin
```
