...
```mdstorm --hide
[ inet:dns:a=(vertex.link,1.1.1.1) ]
```
...
```mdstorm --hide
[ inet:dns:rev=( 1.2.3.4, hurr.com ) inet:dns:rev=( 5.6.7.8, derp.net ) ]
```
...
```mdstorm --hide
[ inet:email=info@vertex.link inet:email=abuse@example.com ( entity:contact=( { "email": "info@vertex.link" } ) :emails+=hello@vertex.link ) ( entity:contact=( { "email": "help@example.com" } ) :emails+=abuse@example.com ) ]
```
...
```mdstorm --hide
[ ( risk:threat=( { "name": "sednit", "reporter:name": "eset" } ) :tag=rep.eset.sednit ) ( risk:threat=( { "name": "waterbug", "reporter:name": "symantec" } ) :tag=rep.symantec.waterbug ) ]
```
...
```mdstorm --hide
[ inet:dns:a=(woot.com,4.4.4.4) inet:dns:ns=(woot.com,ns1.woot.com) inet:dns:mx=(woot.com,mx.woot.com) ]
```
...
```mdstorm --hide
[ inet:dns:a=( example.com, 12.34.56.78 ) inet:dns:aaaa=( example.com, 2606:6080:1001:17:621b:590c:ee14:76e8 ) ]
```
...
```mdstorm --hide
$file={ [ file:bytes=( { "sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" } ) ] } [ it:exec:file:write=( { "sandbox:file": $file, "exe": $file } ) it:exec:mutex:add=( { "sandbox:file": $file, "name": "longlongago" } ) ]
```
...
```mdstorm --hide
[ entity:name=swanson file:path='c:\users\swanson\dev\projects\getthesnacks\release\getthesnacks.pdb' ]
```

```mdstorm --hide
entity:name=swanson -> file:path.base
```
...
```mdstorm --hide
$threat1={ [ risk:threat=( { "name": "swanson", "reporter:name": "thesilence" } ) ] } $threat2={ [ risk:threat=( { "name": "maeve", "reporter:name": "thesilence" } ) ] } [ entity:relationship=( { "source": $threat1, "target": $threat2, "name": "access broker" } ) ]
```
...
```mdstorm --hide
[ inet:dns:a=( hurr.com, 5.5.5.5 ) inet:dns:a=( derp.net, 7.7.7.7 ) ]
```
...
```mdstorm --hide
[ inet:dns:ns=(woot.com,ns1.woot.com) inet:dns:ns=(woot.com,ns2.woot.com) ]
```
...
```mdstorm --hide
[ crypto:x509:cert=( { "sha256": "b1ee0920c4f4d5ded50732d15186eae5357eb743cbbb98ee935bd0791520b5f7" } ) :md5=6b9fdcadf7ea5de1f1402a1cb62c7a65 :sha1=0008e9f15b9ff20de82c276cc54ec9dc094f54f8 :identities:fqdns=(aliceplants.com,) ]
```
...
```mdstorm --hide
[ inet:whois:record=( { "fqdn": "woot.com", "created": "2017/07/17" } ) inet:dns:a=( woot.com, 12.12.12.12 ) ]
```
...
```mdstorm --hide
$file={ [ file:bytes=( { "sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" } ) ] } [ inet:dns:request=( { "client:exe": $file, "query:name": "woot.com" } ) it:exec:file:write=( {  "exe": $file, "path": "c:/windows/scvhost.exe" } ) it:exec:file:read=( { "exe": $file, "path": "c:/windows/autoexec.bat" } ) ]
```
...
```mdstorm --hide
[ inet:dns:a=(woot.com,8.8.8.8) inet:flow=( { "server": "tcp://8.8.8.8" } ) inet:banner=('tcp://8.8.8.8:1234','woo haha') ]
```
...
```mdstorm --hide
[ inet:http:request:header=(referer,https://www.google.com/) inet:http:request:header=(referer,'/some/partial/uri') ]
```
...
```mdstorm --hide
[ inet:fqdn=woot.com +#rep.microsoft.volt_typhoon ]
```
...
```mdstorm --hide
[ file:bytes=( { "sha256": "b6e7343b9250ad141db66c7bb2cd18c8a6939ff6cdff616683ba6b6a4ff7aa91" } ) +#cno.mal ]
```
...
```mdstorm --hide
[ file:bytes=( { "sha256": "89f250599e09f8631040e73cd9ea5e515d87e3d1d989f484686893becec1a9bc" } )  +#rep.alienvault.bisonal +#rep.malwarebazaar.bisonal +#rep.malwarebazaar.3p.intezer.bisonal ]
```
...
```mdstorm --hide
[ file:bytes=( { "sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" } ) +#cno.ttp.phish.attach ]
```
...
```mdstorm --hide
$file={ [ file:bytes=( { "sha256": "c6a9db52a3855d980a7f383dbe2fb70300a12b7a3a4f0a995e2ebdef769eaaca" } ) ] } [ it:exec:file:write=( { "sandbox:file": $file, "path": "c:/windows/system32/udhisapi.dll" } ) +#rep.eset.xtunnel ]
```
...
```mdstorm --hide
[ inet:ip=123.120.19.47 inet:server=tcp://58.248.57.236:443 inet:flow=( { "server": "tcp://175.45.176.21:9999" } ) +#rep.foo.bar ]
```
...
```mdstorm --hide
[ risk:threat=( { "name": "apt29", "reporter:name":"google" } ) +(used)> { [ meta:technique=( { "name": "t1566 - phishing (enterprise)", "reporter:name": "mitre" } ) ] } ]
```
...
```mdstorm --hide
[ doc:report=( { "publisher:name": "palo alto", "title": "bad stuff happened last week" } )  +(refs)> { [ inet:fqdn=woot.com ] } ]
```
...
```mdstorm --hide
[ inet:whois:iprecord=( { "name": "LVLT-GOGL-8-8-8", "net": "8.8.8.0-8.8.8.255" } ) ] [ +(has)> { [ inet:ip=8.8.8.8 ] } ]
```
...
```mdstorm --hide
[ inet:dns:a=(woot.com,12.12.12.12) ] [ <(seen)+ { [ meta:source=( { "name": "alienvault api" } ) meta:source=( { "name": "virustotal api" } ) ] } ]
```
...
```mdstorm --hide
[ inet:fqdn=woot.com ] [ <(refs)+ { [ doc:report=( { "publisher:name": "zscaler", "title": "someone registered a domain" } ) ] } ] [ <(seen)+ { [ meta:source=( { "name": "nettools api" } ) ] } ]
```
...
```mdstorm --hide
[ risk:threat=( { "name": "gamaredon", "reporter:name": "sentinelone" } ) +(used)> { [ it:software=( { "name": "pterodo", "reporter:name": "sentinelone" } ) ] } +(targeted)> { [ ou:org=( { "name": "hetman petro sahaidachnyi national ground forces academy" } ) ] } ]
```
...
```mdstorm --hide
$id={ [ it:sec:cve=cve-2024-21762 ] } [ risk:vuln=( { "id": $id, "reporter:name": "nist" } ) <(used)+ { [ risk:threat=( { "name": "flax typhoon", "reporter:name": "fbi" } ) ] } ]
```
...
```mdstorm --hide
[ inet:whois:iprecord=( { "net": "89.249.65.0-89.249.65.255", "asn": "1234", "registrar:name": "ozzie internet" } ) +(has)> { [ inet:ip=89.249.65.153 ] } ]
```
...
```mdstorm --hide
[ inet:ip=79.24.16.22 inet:dns:a=( example.com, 79.24.16.22 ) <(seen)+ { [ meta:source=( { "name": "alienvault api" } ) ] } ]
```
...
```mdstorm --hide
[ ( ou:org=( { "name": "black cat security" } ) ) ( entity:contact=( { "name": "ron", "type": "black_cat_security.employee" } ) :org={ ou:org:name="black cat security" } :title="chief snack officer" )  ]
```
...
```mdstorm --hide
[ inet:dns:a=( hurr.com, 24.24.24.24 ) inet:dns:a=( derp.net, 127.0.0.1 ) ]
```
...
```mdstorm --hide
[ inet:whois:record=( { "fqdn": "vertex.link", "created": "2016/01/01" } ) inet:dns:a=( vertex.link, 44.44.44.44 ) ]
```
...
```mdstorm --hide
[ file:bytes=( { "sha256": "5fd8153dbb4620ab589aaa83815afce34135e5a0a5af10876fb3b0fff344c64b" } ) :md5=ed64dba976760b2acc3b964a87c13585 :sha1=05c281d9ecc93bcd0261faec2bc95e7851b7e93f :mime=application/pdf :name=report.pdf ]
```
...
```mdstorm --hide
$file= { [ file:bytes=( { "sha256": "96a204e8533f829b32b1c6da03578e5275b613d7fa2bde9dcfdd669f71df4eed" } ) ] } [ inet:dns:request=( { "query:name": "defensewar.org", "sandbox:file": $file } ) ]
```
...
```mdstorm --hide
[ doc:report=( { "publisher:name": "talos", "title": "news about a domain" } ) +(refs)> { [ inet:fqdn=woot.com ] } ]
```
...
```mdstorm --hide
$code={ [ iso:3166:alpha2=my ] } [ risk:threat=( { "name": "apt41", "reporter:name": "google" } ) +(used)> { [ it:software=( { "name": "blackcoffee", "reporter:name": "google" } ) ] } +(targeted)> { [ pol:country=( { "code": $code, "name": "malaysia" } ) ] } ]
```
...
```mdstorm --hide
$id={ [ it:sec:cve=cve-2012-0158 ] }  [ risk:vuln=( { "id": $id, "reporter:name": "nist" } ) <(used)+ { [ risk:threat=( { "name": "apt5", "reporter:name": "google" } ) ] } <(refs)+ { [ doc:report=( { "publisher:name": "google", "title": "who is jonh lu?" } ) ] } ]
```
