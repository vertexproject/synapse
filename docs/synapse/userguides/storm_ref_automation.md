...
```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'virustotal.file.enrich', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'virustotal.file.behavior', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'virustotal.domain.relationships', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'virustotal.ip.relationships', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'alienvault.otx.files', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'alienvault.otx.domain', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'alienvault.otx.pdns', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'alienvault.otx.ip', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'nettools.dns', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'nettools.whois', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'maxmind', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'censys.platform.hosts.enrich', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

<a id="storm-ref-automation"></a>
...
- [Triggers and Cron](storm_ref_automation.md#auto-triggers-cron)
- [Macros](storm_ref_automation.md#auto-macros)
- [Dmons](storm_ref_automation.md#auto-dmon)
...
> [!TIP]
> For a detailed discussion of permissions in Synapse, refer to the [Synapse Admin Guide](../adminguide.md).

### Scope

Automation components vary with respect to where they reside and execute within Synapse; some elements are global (within a Cortex) while some reside and execute within a specific [View](../glossary.md#gloss-view). Organizations that leverage multiple views for their Synapse architecture or that make use of Synapse's fork and merge capabilities should refer to the sections below for information on how views and layers may impact automation.

> [!TIP]
> For a more general discussion of views and layers, including a discussion of forking and merging views, see the [Views and Layers](views_layers.md#userguide_views_layers) section of the Synapse User Guide.
...
In addition to general testing of your Storm code (and Storm logic!), using the ["Try" Operator](storm_ref_data_mod.md#edit-try) when adding or modifying data will ensure your Storm code will warn and continue if it encounters bad data vs. generating an error and halting.

We also encourage you to build additional error-checking and error-handling into your automation as appropriate.

Users should keep the [Storm Operating Concepts](storm_ref_intro.md#storm-op-concepts) in mind when writing automation. Automation frequently operates on nodes or other Synapse data; knowing what nodes are in the Storm pipeline for your automation will help significantly with troubleshooting any issues.

<a id="auto-triggers-cron"></a>
...
> [!NOTE]
> The use of sensitive information (e.g., credentials, API keys, etc.) in the Storm that is executed by a trigger or cron job is **strongly discouraged**. If your automation needs to make use of any sensitive information, we recommend creating a [Power-Up](../power_ups.md) with associated Storm commands. The Power-Up architecture allows users to run commands that make use of sensitive information while ensuring that information is not exposed. See the [Rapid Power-Up Development](../devguides/power-ups.md) section of the [Synapse Developer Guide](../devguide.md) for details.
...
Cron in Synapse is similar to the well-known cron utility. Cron jobs execute their associated Storm on a specified schedule. Jobs can be written to execute once (using the [cron.at](storm_ref_cmd.md#storm-cron-at) command) or on a recurring basis (using [cron.add](storm_ref_cmd.md#storm-cron-add)).

> [!TIP]
> When scheduling cron jobs, Synapse interprets all times as UTC.

<a id="auto-cron-config"></a>
...
- **Permissions.** Cron jobs execute with the privileges of a designated user (by default, the user who creates the cron job). We strongly encourage the use of least privilege; the cron job's account should have the permissions required to execute the associated Storm, but no more. One option is for organizations to create a dedicated account for use with automation in Synapse.
  ...
  Users (and roles) cannot create or manage cron jobs by default. The various `cron.*` permissions must be granted to users or roles that should be allowed to work with cron jobs in your environment. See the [Cortex permissions](../adminguide.md#admin_cortex_perms) section of the [Synapse Admin Guide](../adminguide.md) for details on the `cron.*` permissions and associated gates.
...
The actions performed by a cron job are limited only by your imagination (and your Storm skills). Examples of common cron use cases include:
...
<a id="auto-cron-syntax"></a>


#### Syntax

In Storm, cron jobs are primarily created and managed using the Storm [cron](storm_ref_cmd.md#storm-cron) commands.
...
- `$auto.iden`: The identifier of the cron job.
- `$auto.type`: The type of automation. For a cron job this value will be `cron`.

These job-specific variables can be referenced by the Storm code executed by the cron job. See [Storm Reference - Advanced - Variables](storm_adv_vars.md#storm-adv-vars) for information on the use of variables in Storm.

<a id="auto-cron-examples"></a>


#### Examples

The examples below illustrate using the Storm [cron](storm_ref_cmd.md#storm-cron) commands (`cron.at`, `cron.add`) to create new cron jobs. See the help for each command for options and additional details.

**One-time (non-recurring) cron job example**

You want to use a one-time cron job that runs during off hours to perform some data cleanup. Specifically, you want a job that will lift all existing `inet:ip` nodes that are IPv4 unicast addresses and are missing geolocation data (e.g., do not have a `:place:loc` property), and query a third-party data source to populate the location at 0200 UTC.
...
```mdstorm --hide
[ inet:ip=192.168.0.100 inet:ip=43.237.15.28 inet:ip=2001:67c:e60:c0c:192:42:116:216 ]
```

```mdstorm --hide
inet:ip:version=4 +:type=unicast -:place:loc | maxmind
```

```mdstorm --hide
cron.at --hour 2 { inet:ip:version=4 +:type=unicast -:place:loc | maxmind }
```
...
> [!TIP]
> The `maxmind` command is added by the Synapse-Enterprise-Maxmind Advanced Power-Up.
...
- **creator** - the user that created the cron job.
- **user** - the user that the job runs as (by default, the user who created the job).
...
> [!TIP]
> Detailed information about individual cron jobs can be retrieved with the [cron.stat](storm_ref_cmd.md#storm-cron-stat) command.
...
```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'misp.sync', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm
cron.add hourly@:00 { misp.sync }
```

> [!TIP]
> The `misp.sync` command is added by the Synapse-MISP Rapid Power-Up. The Power-Up is available to Synapse [open-source users](https://vertex.link/blogs/synapse-power-ups/) and Enterprise customers.

**Custom schedule example**

You routinely attempt to download samples of newly identified malware from a third party malware repository. Because new samples are not always available in repositories right away, you want to periodically identify malicious files (`file:bytes` nodes tagged `#cno.mal`) where you are missing the sample (e.g., the sample is not present in Synapse's Axon storage), and attempt to download them again. You want to run the job three times a week (every Tuesday, Thursday, and Saturday at 2000 UTC).

```mdstorm --hide
$pkg=({'name': 'docs', 'version': '0.0.1', 'commands': [{'name': 'malwarebazaar.download', 'storm': ${}}]}) $lib.print($pkg) $lib.pkg.add($pkg)
```

```mdstorm --hide
file:bytes#cno.mal -$lib.axon.has(:sha256) | malwarebazaar.download
```

```mdstorm
cron.add weekly/tue,thu,sat@20:00 { file:bytes#cno.mal -$lib.axon.has(:sha256) | malwarebazaar.download }
```

> [!TIP]
> The `malwarebazaar.download` command is added by the Synapse-MalwareBazaar Rapid Power-Up.

<a id="auto-triggers"></a>
...
- **Execution.** Triggers fire when a change occurs within Synapse - that is, when a **write** operation occurs. Because triggers reside (and execute) within a **view**, they fire on changes to the view's writable (topmost) **layer**. A trigger will **not** fire on changes made in an underlying layer (i.e., of a parent view).
  ...
  Similarly, triggers do not operate retroactively on existing data. If you write a new trigger to fire when the tag `my.tag` is applied to a `crypto:hash:md5` node, the trigger will have no effect on existing `crypto:hash:md5` nodes that already have the tag.
...
- **Permissions.** Triggers execute with the privileges of a designated user (by default, the user who creates the trigger). We strongly encourage the use of least privilege; the trigger's account should have the permissions required to execute the associated Storm, but no more. One option is for organizations to create a dedicated account for use with automation in Synapse.
  ...
  Triggers **run** as a specified user, but they **execute** based on changes to data in Synapse. This means that a lower-privileged user could make a change (such as creating a node) that causes a higher-privileged trigger to execute and perform actions that the user would not be able to do themselves.

  Users (and roles) cannot create or manage triggers by default. The various `trigger.*` permissions must be granted to users or roles that should be allowed to work with triggers in your environment. See the [Cortex permissions](../adminguide.md#admin_cortex_perms) section of the [Synapse Admin Guide](../adminguide.md) for details on the `trigger.*` permissions and associated gates.
...
<a id="auto-trigger-syntax"></a>


#### Syntax

In Storm, triggers are created, modified, viewed, enabled, disabled, and deleted using the Storm [trigger](storm_ref_cmd.md#storm-trigger) commands. In [Optic](../glossary.md#gloss-optic), triggers can also be managed through the [Admin Tool](../glossary.md#gloss-admin-tool) or the [Workspaces Tool](../glossary.md#gloss-workspaces-tool).
...
- `$auto.iden`: The identifier of the Trigger.
- `$auto.type`: The type of automation. For a trigger this value will be `trigger`.
- `$auto.opts`: Dictionary containing trigger-specific runtime information. This includes the following keys:
  - `$auto.opts.form`: The form of the triggering node.
  - `$auto.opts.propfull`: The full name of the property that was set on the node. Only present on `prop:set` triggers.
  - `$auto.opts.propname`: The relative name of the property that was set on the node. Does not include a leading `:`. Only present on `prop:set` triggers.
  - `$auto.opts.tag`: The tag which caused the trigger to fire. Only present on `tag:add` and `tag:del` triggers.
  - `$auto.opts.user`: The iden of the user who caused the trigger to fire. Due to async triggers and potential remote API use cases, trigger code that is using `$auto.opts.user` should be capable of handling the case where the user indicated by this value is not present in the Cortex.
  - `$auto.opts.valu`: The value of the triggering node.
  - `$auto.opts.verb`: The name of the light edge. Only present on `edge:add` and `edge:del` triggers.
  - `$auto.opts.n2nid`: The node id (nid) of the node on the other end of the edge. Only present on `edge:add` and `edge:del` triggers.

These trigger-specific variables can be referenced by the Storm code executed by the trigger. See [Storm Reference - Advanced - Variables](storm_adv_vars.md#storm-adv-vars) for information on the use of variables in Storm.

<a id="auto-trigger-examples"></a>


#### Examples

The examples below illustrate using the Storm [trigger.add](storm_ref_cmd.md#storm-trigger-add) command to create new triggers. See the command help for options and additional details.

Recall from the [Storm Operating Concepts](storm_ref_intro.md#storm-op-concepts) that Storm operations are chained together to act as a pipeline through which nodes are sent and acted upon. Regardless of the type of trigger (`prop:set`, `tag:add`), the **node** associated with the trigger event is sent **inbound** to the Storm executed by the trigger.
...
You have identified a handful of IP addresses that are used as sinkhole infrastructure by various researchers and security organizations. You have tagged the `inet:ip` nodes `#cno.infra.dns.sink.hole` to reflect this. You want to add a trigger that will fire when any of these IP addresses are set as the `:ip` property of an `inet:dns:a` record, and tag the associated FQDN as sinkholed (`#cno.infra.dns.sink.holed`). Because the trigger is a simple one, you want to run it inline (i.e., you do not need the `--async` option).

```mdstorm --hide
inet:dns:a:ip +{ -> inet:ip +#cno.infra.dns.sink.hole } :fqdn -> inet:fqdn [ +#cno.infra.dns.sink.holed ]
```

```mdstorm
trigger.add prop:set --name 'Tag sinkholed FQDNs based on known sinkhole IPs' --prop inet:dns:a:ip { +{ -> inet:ip +#cno.infra.dns.sink.hole } :fqdn -> inet:fqdn [ +#cno.infra.dns.sink.holed ] }
```
...
- **creator** - the user that created the trigger.
- **user** - the user that the trigger runs as (by default, the user who created the trigger).
- **iden** - the trigger's identifier (iden).
- **view** - the first eight characters of the identifier (iden) of the view in which the trigger resides / runs.
...
- **object** - the object the trigger operates on (in this case, the `:ip` property of an `inet:dns:a` node). May reference multiple objects (e.g., if a `tag:add` trigger only fires when the tag is added to a specific form, both the tag and form will be listed).
- **storm query** - the Storm to be executed when the trigger fires.

**node:add example**

Whenever an IP node is added to Synapse, you want to immediately retrieve the associated Autonomous System (AS), geolocation, and DNS PTR data using various Storm commands.

```mdstorm
trigger.add node:add --name 'Basic IP enrichment' --form inet:ip { maxmind | nettools.dns }
```

> [!TIP]
> For `node:add` and `node:del` triggers, the node that caused the trigger to fire is sent inbound to the trigger's Storm.
>
> The `maxmind` and `nettools.dns` commands are added to Synapse by the Synapse-Enterprise-Maxmind and Synapse-Nettools Advanced Power-Ups, respectively.

**edge:add example**

You are using `risk:attack` nodes in Synapse to represent attacks, and linking the attack to key objects used in the attack with a `-(used)>` light edge. (For example, a `risk:attack` representing a phishing attempt can be linked to the relevant email message (`file:bytes` node) or email metadata nodes (such as `inet:email:message`, `inet:email:message:attachment`, or `inet:hyperlink`).

If the object `-(used)>` in the attack has an associated TTP tag (such as `#cno.ttp.phish.attach` to represent a malicious attachment used in a phishing attack), you want to pivot to the technique (`meta:technique`) represented by the tag (e.g., "Spear phishing attachment"), and create another `-(used)>` edge to show the attack used the specified technique.

```mdstorm
trigger.add edge:add --name 'Link technique with attack'--verb used --form risk:attack { $attack=$node { yield $auto.opts.n2nid -> # -> meta:technique:tag [ <(used)+ $attack ] } }
```

> [!TIP]
> Recall that edges have a direction, regardless of whether the edge relationship is specified left to right or right to left in Storm (e.g., `n1 -(used)> n2` or `n2 <(used)- n1`).
>
...
>
> The trigger above uses the trigger-specific `$auto.opts.n2nid` variable to reference the `n2` node that was linked by the `-(used)>` edge which caused the trigger to fire.
...
// Lift the n2 node that was just linked
yield $auto.opts.n2nid

// Pivot to the syn:tag nodes associated with any tags on n2
-> #

// Pivot to any techniques associated with those tags
-> meta:technique:tag

// Create a 'used' edge between the technique(s) and the risk:attack node
[ <(used)+ $attack ]
```
...
```mdstorm
trigger.add tag:add --name 'Push malware tags from file to hashes' --form file:bytes --tag cno.mal.** { tee { :md5 -> crypto:hash:md5 } { :sha1 -> crypto:hash:sha1 } { :sha256 -> crypto:hash:sha256 } { :sha512 -> crypto:hash:sha512 } | [ +#$auto.opts.tag ] }
```
...
```mdstorm
trigger.add tag:add --name 'Untag hashes when untagging a file' --form file:bytes --tag cno.mal.** { tee { :md5 -> crypto:hash:md5 } { :sha1 -> crypto:hash:sha1 } { :sha256 -> crypto:hash:sha256 } { :sha512 -> crypto:hash:sha512 } | [ -#$auto.opts.tag ] }
```
...
> [!TIP]
> In addition to macros, [Optic](../glossary.md#gloss-optic) includes features such as [Node Actions](/docs/synapse-enterprise-optic/latest/user_interface/userguides/custom_environ.md#create-a-custom-node-action) and Bookmarks that can be used to save and execute Storm on demand.

Macros can be executed on demand, but are also commonly used with (called by) triggers or cron jobs. In particular, where a trigger or cron job executes longer or more detailed Storm, it may be easier to store and maintain that Storm within a macro instead of within the trigger or cron job itself.

As a stored Storm query, a macro is meant to operate on nodes. You must either specify (lift) the nodes to be operated on within the macro itself, or provide the nodes as input to the macro (i.e., by sending the results of an existing Storm query to the [macro.exec](storm_ref_cmd.md#storm-macro-exec) command that runs the macro).
...
> [!NOTE]
> The use of sensitive information (e.g., credentials, API keys, etc.) in the Storm that is executed by a macro is **strongly discouraged**. If your automation needs to make use of any sensitive information, we recommend creating a [Power-Up](../power_ups.md) with associated Storm commands. The Power-Up architecture allows users to run commands that make use of sensitive information while ensuring that information is not exposed. See the [Rapid Power-Up Development](../devguides/power-ups.md) section of the [Synapse Developer Guide](../devguide.md) for details.

<a id="auto-macro-config"></a>
...
<a id="auto-macro-examples"></a>


### Examples

Macros are simply stored Storm, and can contain any Storm that simplifies your analysis workflow - from the very simple to longer, more detailed queries. The command `macro.exec <macro_name>` is much simpler to type than Storm that you have to recreate each time. The examples below are relatively simple for illustrative purposes; that said, any Storm that you want to easily and consistently run on a regular basis, no matter how simple or complex, is a candidate for a macro (or potentially for other automation).

The examples below show creating macros using the `macro.set` command.

**Self-contained macro - example**

You have a set of IP addresses that you have identified as sinkholes (tagged `#cno.infra.dns.sink.hole`). By analyzing associated DNS and domain whois data, you have tagged several FQDNs resolving to the IPs as sinkholed domains (`#cno.infra.dns.sink.holed`). You want to periodically check the sinkhole IPs for newly sinkholed FQDNs. To do this, you create a macro called `sinkhole.check` that will:

- query a passive DNS data source to check for new FQDNs resolving to the various sinkhole IPs;
...
```mdstorm
macro.set sinkhole.check { $now=$lib.time.now() inet:ip#cno.infra.dns.sink.hole | alienvault.otx.pdns --yield | -> inet:fqdn :zone -> inet:fqdn -#cno.infra.dns.sink.holed | uniq | nettools.dns | nettools.whois | [ +#int.review.sinkhole=$now ] }
```
...
// Lift IP nodes tagged as sinkholes
...
> [!TIP]
> This macro is self-contained (does not expect inbound nodes); the macro's Storm lifts the nodes that the macro operates on. (This does not prevent a user from sending nodes to the macro - which may cause unexpected effects - but as written, the macro can execute on its own.)
>
...
>
> The `alienvault.otx.pdns` command is installed by the Synapse-AlienVault Rapid Power-Up. The `nettools.*` commands are installed by the Synapse-Nettools Advanced Power-Up.

**Macro that takes nodes as input - example**

One of Synapse's strengths is the ability to retrieve data from a wide range of external sources, typically by using a Storm command provided by a [Power-Up](../power_ups.md). The Storm commands allow an analyst to ingest data into Synpase and represent disparate data in a consistent manner for review and analysis. An analyst who wants to investigate an object (such as an indicator) in Synapse will commonly run multiple Storm commands to retrieve additional data about the object from any / all data sources (a process commonly known as enrichment).
...
```mdstorm --hide
macro.set enrich { +(crypto:hash:md5 or crypto:hash:sha1 or crypto:hash:sha256 or inet:fqdn or inet:ip) switch $node.form { ("crypto:hash:md5", "crypto:hash:sha1"): { { | virustotal.file.enrich | virustotal.file.behavior } }  "crypto:hash:sha256": { { | virustotal.file.enrich | virustotal.file.behavior | alienvault.otx.files } }  "inet:fqdn": { { +:iszone=1 +:issuffix=0 { | nettools.dns --type A AAAA CNAME MX NS SOA TXT | nettools.whois | virustotal.domain.relationships resolutions communicating_files | alienvault.otx.domain | alienvault.otx.pdns } }  { +:iszone=0 +:issuffix=0 { | nettools.dns --type A AAAA CNAME | virustotal.domain.relationships resolutions communicating_files | alienvault.otx.domain | alienvault.otx.pdns } } }  "inet:ip": { +:type=unicast  { | maxmind | nettools.dns | nettools.whois | virustotal.ip.relationships resolutions communicating_files | censys.platform.hosts.enrich | alienvault.otx.ip | alienvault.otx.pdns } }  *: { } } }
```
...
switch $node.form {
  ...
  "inet:fqdn": {
      ...
      {
          +:iszone=1 +:issuffix=0
          { | nettools.dns --type A AAAA CNAME MX NS SOA TXT | nettools.whois | virustotal.domain.relationships resolutions communicating_files | alienvault.otx.domain | alienvault.otx.pdns }
      }

      // For FQDN subdomains
      {
          +:iszone=0 +:issuffix=0
          { | nettools.dns --type A AAAA CNAME | virustotal.domain.relationships resolutions communicating_files | alienvault.otx.domain | alienvault.otx.pdns }
      }
  }
  ...
  "inet:ip": {
      ...
      +:type=unicast
      { | maxmind | nettools.dns | nettools.whois | virustotal.ip.relationships resolutions communicating_files | censys.platform.hosts.enrich | alienvault.otx.ip | alienvault.otx.pdns }
  }
...
Instead of running multiple Storm commands, an analyst can now run a single macro that will perform enrichment by querying all available data sources that can provide information about a particular kind of indicator.
...
>
> The example commands used in the macro are all installed by various Synapse [Power-Ups](../power_ups.md).
>
> The macro makes use of a [Switch Statement](storm_adv_control.md#flow-switch) (part of Storm's [control flow](storm_adv_control.md) features) to handle different kinds of nodes (forms).
>
> Macros run inline by default; when a user calls a macro, the full macro code executes to completion. For enrichment-type macros, depending on the number of data sources queried and nodes created, enrichment can take some time to run, which can cause the Synapse interface (the Storm CLI or the Optic UI) to block for the user until the Storm executes in full. The macro code can be run asynchronously using the Storm [background](storm_ref_cmd.md#storm-background) command. This unblocks the CLI / UI, but the full set of results from enrichment will not be available until the macro finishes executing.
...
```mdstorm --hide
auth.user.add 'maeve' --email maeve@vertex.link
```

```mdstorm --hide
auth.role.add interns
```

```mdstorm --hide
auth.role.add 'cattribution analysts'
```

```mdstorm --hide
auth.user.grant 'ron the cat' 'cattribution analysts'
```

```mdstorm --hide
auth.user.grant maeve interns
```

Make user "ron the cat" co-admin of the `sinkhole.check` macro:

```mdstorm
$user=$lib.auth.users.byname('ron the cat') $iden=$user.iden $lib.macro.grant(sinkhole.check,users,$iden,3)
```

Allow the "cattribution analysts" role to edit the `enrich` macro:

```mdstorm
$role=$lib.auth.roles.byname('cattribution analysts') $iden=$role.iden $lib.macro.grant(enrich,roles,$iden,2)
```

Deny the "interns" role access to the `enrich` macro:

```mdstorm
$role=$lib.auth.roles.byname(interns) $iden=$role.iden $lib.macro.grant(enrich,roles,$iden,0)
```

> [!TIP]
> Easy permissions allow you to assign common permissions based on a corresponding integer value:
>
> 0 - Deny 1 - Read (including execute) 2 - Edit (modify content; includes Read) 3 - Admin (delete the object and modify its permissions; includes Edit)
>
> See the [Synapse Admin Guide](../adminguide.md) for a detailed discussion of permissions, including [easy permissions](../adminguide.md#admin_bkd_easyperms) (easy perms).
...
A [Dmon](../glossary.md#gloss-dmon) is a long-running or recurring query or process that runs continuously in the background, similar to a traditional Linux or Unix daemon.
...
- `$auto.iden`: The identifier of the Dmon.
- `$auto.type`: The type of automation. For a Dmon this value will be `dmon`.
