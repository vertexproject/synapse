...
```mdstorm --hide
[ inet:dns:a=(woot.com, 54.173.9.236) :seen=('2016/12/28 20:46:31.000', '2016/12/28 20:46:31.001') ]
```

```mdstorm
inet:dns:a=(woot.com, 54.173.9.236) $lib.print($node) | spin
```

Print the value of `$node` for an `inet:fqdn` node with tags present:

```mdstorm --hide
[ inet:fqdn=aunewsonline.com +#rep.mandiant.apt1 +#cno.infra.dns.sink.hole.kleissner=(2013/11/26, 2016/11/26) ]
```

```mdstorm
inet:fqdn=aunewsonline.com $lib.print($node) | spin
```
...
```mdstorm --hide
[ inet:fqdn=ns1.example.com inet:whois:record=( { "fqdn": "example.com", "created": "2024/03/27 03:00", "updated": "2026/03/22 11:37" } ) inet:whois:record=( { "fqdn": "example.com", "created": "2024/03/27 03:00", "updated": "2024/09/18 22:46" } ) ]
```

```mdstorm
inet:fqdn=ns1.example.com $ns=$node :zone -> inet:whois:record:fqdn [ :nameservers+=$ns ]
```
...
```mdstorm --hide
[ risk:threat=( { "name": "sparkling unicorn", "reporter:name": "vertex" } ) ]
```

```mdstorm
risk:threat:name='sparkling unicorn' $actor=$node [ ( risk:compromise=( { "name": "very bad compromise", "reporter:name": "vertex" } ) :actor=$actor ) ]
```
...
```mdstorm
inet:dns:a=(woot.com,54.173.9.236) $lib.print($node.form) | spin
```
...
```mdstorm
inet:fqdn=aunewsonline.com $lib.print($node.globtags('*')) | spin
```
...
```mdstorm --hide
[inet:fqdn=aunewsonline.com  +#rep.symantec.commentcrew +#rep.mcafee.commentcrew +#rep.crowdstrike.commentpanda +#cno.threat.t83.own +#foo.bar.baz +#faz.baz +#foo.derp]
```

```mdstorm
inet:fqdn=aunewsonline.com $lib.print($node.globtags(cno.threat.t*)) | spin
```
...
```mdstorm
inet:fqdn=aunewsonline.com $lib.print($node.globtags(rep.*.*)) | spin
```

Print all sub-tags for any tags starting with `foo` applied to the current node:

```mdstorm
inet:fqdn=aunewsonline.com $lib.print($node.globtags(foo.**)) | spin
```
...
```mdstorm
inet:ip=54.173.9.236 $lib.print($node.is(inet:fqdn)) | spin
```

Print the Boolean value for whether a node is either an `inet:fqdn` or an `inet:ip` form:

```mdstorm
inet:ip=54.173.9.236 $lib.print($node.is((inet:fqdn, inet:ip))) | spin
```
...
```mdstorm
risk:threat:name='sparkling unicorn' $lib.print($node.is(entity:actor)) | spin
```

Print the Boolean value for whether a node is an `it:host:account` form, or any form that extends that form:

```mdstorm --hide
[ it:host:windows:account=( { "username": "ron the cat", "home": "c:\\users\\ron the cat" } ) ]
```

```mdstorm
it:host:windows:account:username='ron the cat' $lib.print($node.is(it:host:account)) | spin
```
...
```mdstorm
inet:dns:a=(woot.com, 54.173.9.236) $lib.print($node.ndef) | spin
```
...
```mdstorm
inet:dns:a=(woot.com, 54.173.9.236) $lib.print($node.repr())  | spin
```

Print the repr of the `:ip` secondary property value of an `inet:dns:a` node:

```mdstorm
inet:dns:a=(woot.com, 54.173.9.236) $lib.print($node.repr(ip)) | spin
```

Print the repr of the `.ip` virtual property of an `inet:server` node:

```mdstorm --hide
$server={ [ inet:server=tcp://67.227.226.240:22 ] } [ inet:flow=( { "server": $server} ) ]
```

```mdstorm
inet:server=tcp://67.227.226.240:22 $lib.print($node.repr('.ip')) | spin
```

Print the repr of the `:server.ip` virtual property of an `inet:flow` node:

```mdstorm
inet:flow:server=tcp://67.227.226.240:22 $lib.print($node.repr(server.ip)) | spin
```

Print the repr of the `.created` meta property of an `inet:dns:a` node:

```mdstorm
inet:dns:a=(woot.com, 54.173.9.236) $lib.print($node.repr('.created')) | spin
```
...
```mdstorm
inet:dns:a=(woot.com, 54.173.9.236) $lib.print($node.value) | spin
```
...
```mdstorm --hide
[ (inet:dns:a=(aunewsonline.com, 67.215.66.149)) (inet:dns:a=(aunewsonline.com, 184.168.221.92)) (inet:dns:a=(aunewsonline.com, 104.239.213.7)) ]
```

```mdstorm
inet:fqdn=aunewsonline.com -> inet:dns:a +:ip=67.215.66.149 -> inet:ip $lib.print($path.links())
```
...
```mdstorm
inet:fqdn=aunewsonline.com -> inet:dns:a -> inet:ip $lib.print($path.links())
```
