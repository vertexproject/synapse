...
- [Subquery](storm_ref_subquery.md#storm-subquery)
- [Subquery Filter](storm_ref_subquery.md#storm-subquery-filter)
- [Using Subqueries to Reference Nodes](storm_ref_subquery.md#subquery-ref-nodes)

<a id="storm-subquery"></a>
...
>
> In addition, when setting or updating a [Variable](../glossary.md#gloss-variable) inside a subquery, the variable can pass back "out" of the subquery and be available to the main Storm query.
>
...
> [!NOTE]
> A subquery is typically used to perform some action related to the Storm query in which it is embedded. But there is no requirement for this to be the case. The subquery can contain any valid Storm, so you could (for example) write a subquery that lifts ten arbitrary email addresses ( `{ inet:email | limit 10 }` ) in the middle of a longer query. There's not much point to this, but Storm will dutifully lift the nodes, discard them (unless the `yield` keyword is used), and continue on.
...
For example, the following query will return zero nodes, even though the `yield` keyword is present. Because no nodes are "inbound" to cause the subquery to execute, the embedded Storm is never run:

```mdstorm
yield { inet:email | limit 10 }
```
...
```mdstorm --hide
[ (inet:ip=12.34.56.78 :place:loc=us.oh) (inet:ip=44.44.44.44) (inet:server=tcp://12.34.56.78:80) (inet:server=tcp://44.44.44.44:443)]
```

```mdstorm --hide
inet:ip { -:place:loc } -> inet:server
```
...
```mdstorm --hide
$file={ [ file:bytes=( { "sha256": "cc5f23669712ce42efa66054acefbe29a967c53e59206fbc78670672ea3978bd" } ) ] } [ inet:fqdn=macsol.org inet:dns:request=( { "client:exe": $file, "query:name": "macsol.org" } ) ]
```
...
- When the `yield` keyword is used, Storm will return the nodes from the subquery first, followed by the nodes from the original working set.

<a id="storm-subquery-filter"></a>
...
For additional detail on subquery filters and examples of their use, refer to the [Subquery Filters](storm_ref_filter.md#filter-subquery) section of the [Storm Reference - Filtering](storm_ref_filter.md#storm-ref-filter) guide.

<a id="subquery-ref-nodes"></a>
...
A common use case is to use a subquery as a simpler way to refer to a [Guid Form](../glossary.md#gloss-guid-form) or [Composite Form](../glossary.md#gloss-comp-form) without needing to enter (type, copy and paste) the form's primary property.
...
```mdstorm --hide
$org = { [ ou:org=( { "name": " vertex" } ) :names+='the vertex project' ] } [ entity:contact=( { "name": "ron the cat", "type": "vertex.employee" } ) :org:name=vertex :org=$org ]
```
...
>
> `ERROR: BadTypeValu: Subquery used as a value yielded too many (>1) nodes. ou:org:name = vertex`

Lift all of the presentations that were given at PIVOTcon 2026:

```mdstorm --hide
$conf={ [ ou:conference=( { "name": "pivotcon 2026", "family": "pivotcon" } ) ] } [ ou:preso=( { "name": "my cool china talk", "activity": $conf } ) ou:preso=( { "name": "this talk is not about china", "activity": $conf } ) ou:preso=( { "name": "a different talk about russia", "activity": $conf } ) ]
```
...
See the [Add or Modify Properties Using Subqueries](storm_ref_data_mod.md#prop-add-mod-subquery) section of the [Storm Reference - Data Modification](storm_ref_data_mod.md#storm-ref-data-mod) document for additional examples of setting properties using subqueries.
