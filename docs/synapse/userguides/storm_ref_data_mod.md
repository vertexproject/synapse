...
```mdstorm
[ inet:ip?=woot.com inet:ip?=22.22.22.22 ]
```
...
```mdstorm --hide
[ inet:ip=127.0.0.1 ] [ :place:loc=us.oh.wilmington ]
```
...
```mdstorm --hide
[ ( ou:org=( { "name": "The Vertex Project",  } ) :websites+=https://vertex.link/ ) ( entity:contact=( { "name": "Ozzie the Pony", "type": "employee.vertex" } ) :org:name='The Vertex Project' ) ]
```

```mdstorm
entity:contact:name='Ozzie the Pony' [ :org={ ou:org:name='The Vertex Project' } ]
```
...
```mdstorm --hide
[ ( ou:org=( { "name": "apple" } ) :names+='apple, inc.') ind:industry=( { "name": "c - manufacturing", "reporter:name": "isic" } ) ind:industry=( { "name": "k - telecommunications, computer programming, consultancy, computing infrastructure, and other information service activities", "reporter:name": "isic" } ) ]
```

```mdstorm
ou:org:name=apple [ :industries+={ ind:industry:name~=manufacturing ind:industry:name~=telecommunications } ]
```
...
```mdstorm --hide
$creator={ [ entity:contact=( { "name": "savage", "type": "vertex.employee" } ) ] } [ doc:report=( { "title": "you're doing it wrong", "publisher:name": "vertex" } ) :creator=$creator :desc='YOU PEOPLE' ]
```
...
```mdstorm --hide
[ inet:fqdn=woot.com inet:ip=1.2.3.4 doc:report=( { "title": "report about bad stuff", "publisher:name": "zscaler" } ) ]
```
...
```mdstorm --hide
[ doc:report=( { "title": "apt1 report", "publisher:name": "mandiant"} ) inet:fqdn=newsonet.net inet:fqdn=staycools.net inet:fqdn=hugesoft.org inet:fqdn=purpledaily.com +#rep.mandiant.apt1 ]
```
...
```mdstorm --hide
[ meta:technique=( { "name": "phishing", "reporter:name": "mitre" } ) risk:threat=( { "name": "forest blizzard", "reporter:name": "microsoft" } ) ]
```
...
```mdstorm --hide
[ doc:report=( { "title": "\"It's all WINNTI\", says researcher", "publisher:name": "bloomberg" } ) ]
```
...
```mdstorm --hide
[ crypto:hash:md5=d41d8cd98f00b204e9800998ecf8427e doc:report=( { "title": "report about bad stuff", "publisher:name": "zscaler" } ) ]
```
...
```mdstorm --hide
[ risk:threat=( { "name": "forest blizzard", "reporter:name": "microsoft" } ) +(used)> { [ meta:technique=( { "name": "phishing", "reporter:name": "mitre" } ) ] } ]
```
...
```mdstorm --hide
[ doc:report=( { "title": "\"It's all WINNTI\", says researcher", "publisher:name": "bloomberg" } ) +(refs)> { inet:fqdn=woot.com } ]
```
...
```mdstorm --hide
[ inet:ip=185.29.8.215 +#cno.infra.anon.tor.exit ]
```
...
```mdstorm --hide
[ inet:fqdn=blackcake.net ] [ +#rep.mandiant.apt1 +#cno.infra.dns.sink.holed ]
```
...
```mdstorm --hide
inet:fqdn=unwashedsound.com [ +#cno.infra.dns.sink.holed=2018/11/27 ]
```
...
```mdstorm --hide
[ inet:fqdn=coffeehausblog.com ]
```

```mdstorm --hide
inet:fqdn=coffeehausblog.com [ +#cno.infra.dns.sink.holed=(2014/11/06, 2016/11/06) ]
```
...
```mdstorm --hide
$tagpropinfo=({"doc": "A risk tagprop."}) $lib.model.ext.addTagProp(_risk, (int, ({})), $tagpropinfo) [inet:fqdn=woot.com +#rep.symantec:_risk=87]
```
...
```mdstorm --hide
[ inet:ip=23.191.200.123 +#cno.infra.anon.tor.exit ]
```
...
```mdstorm --hide
inet:fqdn=woot.com [ -#rep.symantec:_risk ]
```
...
```mdstorm --hide
inet:fqdn#rep.mandiant.apt1 [-#rep.eset]
```
