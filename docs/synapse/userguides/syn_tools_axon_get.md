<a id="syn-tools-axon-get"></a>


# axon.get

The Synapse `axon.get` command can be used to retrieve (download) one or more files from a storage Axon (see Axon in the [Synapse Devops Guide](../devopsguide.md#devopsguide)).
...
- `OUTPUT` is the optional directory path where the downloaded file(s) should be written.
  ...
  - It is not possible to specify multiple `-o` options with a single `axon.get` command (i.e., a different `-o` option with each `-l HASH`, for example). If multiple `-o` options are specified, the last `OUTPUT` path specified will be used.
...
> [!NOTE]
> This tool was previously run using `synapse.tools.pullfile`, which was removed in Synapse 3.0.0. See [CLI Tool Changes](../300_changes/devops-cli-tools.md#vtx_300_devops-cli-tools).
