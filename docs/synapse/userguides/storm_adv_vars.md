...
```mdstorm
$threshold=5 $lib.print($threshold)
```
...
```mdstorm --hide
$file1={ [ file:bytes=( { "sha256": "fe1bef0d799269eb8d4a23afc523301a8c40952fa81bb43d1011e6beeccfd44e" } ) ] } [ it:av:scan:result=( { "target": $file1, "time": "2026/04/22 09:33:07", "scanner:name": "foo", "verdict": "malicious" } ) it:av:scan:result=( { "target": $file1, "time": "2026/04/22 09:33:07", "scanner:name": "bar", "verdict": "malicious" } ) it:av:scan:result=( { "target": $file1, "time": "2026/04/22 09:33:07", "scanner:name": "baz", "verdict": "malicious" } ) it:av:scan:result=( { "target": $file1, "time": "2026/04/22 09:33:07", "scanner:name": "hurr", "verdict": "malicious" } ) it:av:scan:result=( { "target": $file1, "time": "2026/04/22 09:33:07", "scanner:name": "derp", "verdict": "malicious" } ) ]
```

```mdstorm --hide
$file2={ [ file:bytes=( { "sha256": "9056653006806f63555b309ef18be3f16a1ddfb559b3ce3c3eb20f1091f3c05a" } ) ] } [ it:av:scan:result=( { "target": $file2, "time": "2026/06/01 14:56:19", "scanner:name": "foo", "verdict": "unknown" } ) it:av:scan:result=( { "target": $file2, "time": "2026/06/01 14:56:19", "scanner:name": "bar", "verdict": "unknown" } ) it:av:scan:result=( { "target": $file2, "time": "2026/06/01 14:56:19", "scanner:name": "baz", "verdict": "suspicious" } ) it:av:scan:result=( { "target": $file2, "time": "2026/06/01 14:56:19", "scanner:name": "hurr", "verdict": "benign" } ) it:av:scan:result=( { "target": $file2, "time": "2026/06/01 14:56:19", "scanner:name": "derp", "verdict": "malicious" } ) ]
```

```mdstorm
$threshold=5 file:bytes +{ -> it:av:scan:result:target +:verdict=malicious }>=$threshold [ +#review ]
```
...
```mdstorm --hide
$platform={ [ inet:service:platform=( { "name": "bluesky", "zone": "bsky.app", "url": "https://bsky.app/" } ) ] }  [ inet:service:account=( { "platform": $platform, "name": "hacks4cats", "$props": { "email": "ron@protonmail.com" } } ) ]
```

```mdstorm
inet:service:account:platform::name=bluesky +:name=hacks4cats $email=:email $lib.print($email)
```
...
```mdstorm
inet:fqdn=mail.mydomain.com $fqdn=$node $lib.print($fqdn)
```
...
```mdstorm --hide
[ inet:dns:a=(mail.mydomain.com, 11.12.13.14) inet:dns:a=(mail.mydomain.com, 25.25.25.25) ( inet:ip=25.25.25.25 :dns:rev=mail.mydomain.com ) ]
```

```mdstorm
inet:fqdn=mail.mydomain.com $fqdn=$node.value $lib.print($fqdn)
```

Find the DNS A records associated with a given domain where the PTR record for the IP matches the FQDN:

```mdstorm
inet:fqdn=mail.mydomain.com $fqdn=$node.value -> inet:dns:a +{ -> inet:ip +:dns:rev=$fqdn }
```
...
```mdstorm --hide
[ crypto:hash:sha256=21101ae64d881df51e46be9e4f693a9029862c5dab8a53d504303c53a5d1d7a3 +#cno.mal.foo +#cno.mal.bar +#cno.threat.baz ]
```

```mdstorm
crypto:hash:sha256=21101ae64d881df51e46be9e4f693a9029862c5dab8a53d504303c53a5d1d7a3 $mytags=$node.tags(cno.mal.*) $lib.print($mytags)
```
...
```mdstorm --hide
[ file:bytes=( { "sha256": "21101ae64d881df51e46be9e4f693a9029862c5dab8a53d504303c53a5d1d7a3", "$props": { "sha1": "b26974e367002bddd3545609fcfd54cbd4d076de", "md5": "0ca6e2ad69826c8e3287fc8576112814" } } ) ]
```

```mdstorm
crypto:hash:sha256=21101ae64d881df51e46be9e4f693a9029862c5dab8a53d504303c53a5d1d7a3 $mytags=$node.tags(cno.mal.*) for $tag in $mytags { -> file:bytes [ +#$tag ] }
```
...
```mdstorm --hide
[ inet:fqdn=evildomain.com +#cno.threat.t20.own=(2019/09/08, 2021/09/08) ]
```
...
```mdstorm --hide
[ ( inet:dns:a=(www.evildomain.com, 1.2.3.4) :seen=(2020/07/12, 2020/12/13) ) ( inet:dns:a=(smtp.evildomain.com, 5.6.7.8) :seen=(2020/04/04, 2020/08/02) ) ( inet:dns:a=(evildomain.com, 12.13.14.15) :seen=(2021/12/22, 2022/12/22) ) ]
```

```mdstorm
inet:fqdn#cno.threat.t20.own $time=#cno.threat.t20.own -> inet:fqdn:zone -> inet:dns:a +:seen@=$time
```
...
```mdstorm
$end=2022-01-02 as time $start=2022-01-01 as time $lib.print(($end - $start))
```

- Add a `duration` to a `time` to get a new `time`:

```mdstorm
$start=2022-01-01 as time $delta=1D as duration $lib.print(($start + $delta))
```
...
```mdstorm --hide
[ ou:org=( { "name": "vertex", "$props": { "place:loc": "us.va" } } ) :names+='the vertex project' ]
```

```mdstorm
$org={ ou:org:name=vertex } $lib.print($org)
```
