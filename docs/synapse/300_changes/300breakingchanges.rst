...
- Package definition ``descr`` fields are renamed to ``desc`` on command definitions, ``optic.actions`` entries, and ``optic.spotlight.extractors`` entries (see :ref:`vtx_300_storm-package-command-desc`). Fix: rename ``descr`` to ``desc`` in each of these spots; a pkgdef that still uses ``descr`` fails schema validation.
