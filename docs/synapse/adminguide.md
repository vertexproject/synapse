...
```mdstorm
auth.user.add ron --email ronthecat@vertex.link
```
...
```mdstorm
auth.user.mod ron --email ron@vertex.link
```

Assign **admin** status to the user "ron_admin":

```mdstorm --hide
auth.user.add ron_admin
```

```mdstorm
auth.user.mod ron_admin --admin (true)
```

Remove **admin** status from user "ron_admin":

```mdstorm
auth.user.mod ron_admin --admin (false)
```

Lock the user account "ron_admin":

```mdstorm
auth.user.mod ron_admin --locked (true)
```
...
```mdstorm
auth.role.add "cattribution analyst"
```
...
```mdstorm
auth.role.mod "cattribution analyst" --name "meow-ware analyst"
```
...
```mdstorm --hide
auth.role.add "a-cat-emic researcher"
```
...
```mdstorm
auth.role.del "meow-ware analyst"
```
...
```mdstorm
auth.user.grant ron "cattribution analyst"
```

Revoke the role "a-cat-emic researcher" from user "ron":

```mdstorm --hide
auth.user.grant ron "a-cat-emic researcher"
```

```mdstorm
auth.user.revoke ron "a-cat-emic researcher"
```
...
```mdstorm
auth.user.addrule ron "!node.prop.set.syn:tag:desc"
```
...
```mdstorm --hide
auth.role.add "senior analysts"
```

```mdstorm
auth.role.addrule "senior analysts" node.tag.add.cno.threat
```

Prevent the "all" role from deleting nodes:

```mdstorm
auth.role.addrule all "!node.del"
```

Prevent the "all" role from deleting nodes, and insert this as the first rule for the role:

```mdstorm
auth.role.addrule --index 0 all "!node.del"
```
...
```mdstorm
auth.user.delrule ron "!node.prop.set.syn:tag:desc"
```

Revoke the rule that allows "junior analysts" to apply tags in the `cno.threat` tag tree:

```mdstorm --hide
auth.role.add "junior analysts"
```

```mdstorm --hide
auth.role.addrule "junior analysts" node.tag.cno.threat
```

```mdstorm
auth.role.delrule "junior analysts" node.tag.cno.threat
```
...
```mdstorm --hide
auth.role.addrule "cattribution analyst" node.tag.add.cno
```

```mdstorm
auth.user.allowed ron node.tag.add.cno
```

Check whether user 'ron' is allowed to apply tags in the `cno` tag tree in the **current layer:**

```mdstorm
auth.user.allowed --gate $lib.layer.get().iden ron node.tag.add.cno
```
...
```mdstorm
auth.user.allowed --gate $lib.view.get().iden ron view.add
```
...
```mdstorm --hide
$lib.model.ext.addFormProp(inet:fqdn, _depr, (str, ({})),({'deprecated': true}))
```

```mdstorm
model.deprecated.lock inet:fqdn:_depr
```

Unlock the `inet:fqdn:_depr` property:

```mdstorm
model.deprecated.lock --unlock inet:fqdn:_depr
```
